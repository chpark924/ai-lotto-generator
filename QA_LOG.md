...
### 120. 생성 결과 화면 — "같은 조건으로 다시 생성" 로딩 표시가 기기 하단 내비게이션 바에 가려짐
- **피드백**: 생성 결과 화면에서 "같은 조건으로 다시 생성"을 누르면, 로딩 중 표시(공 5개 애니메이션 + "다시 생성하는 중..." 문구)가 화면 맨 아래에서 잘려서 공 몇 개만 보이고 문구는 아예 안 보임. 스크린샷 4장으로 재현 확인(정상 상태 2장 + 로딩 중 잘린 상태 2장).
- **원인**: `app/generate/result.tsx`의 `ScrollView`는 `contentContainerStyle`에 `paddingBottom: insets.bottom + 24`를 이미 주고 있어 안전영역 자체는 반영돼 있었다. 문제는 스크롤 위치였다 — 사용자가 버튼을 누르는 시점엔 이미 화면을 맨 아래(버튼이 보이는 지점)까지 스크롤해둔 상태인데, `isRegenerating`이 `true`가 되면서 한 줄짜리 버튼(`regenerateButton`)이 공 5개 행 + 문구가 있는 더 높은 로딩 블록(`regenerateLoadingContainer`)으로 바뀐다. 콘텐츠 전체 높이가 늘어나는데 스크롤 위치는 예전 최하단 값 그대로 유지되므로, 새로 늘어난 아랫부분(로딩 문구·안전영역 패딩)이 예전 스크롤 최하단보다 더 아래로 밀려나 뷰포트 밖(기기 내비게이션 바 쪽)으로 넘어가 버렸다. 안전영역 계산 자체는 문제가 아니었고, "콘텐츠가 커졌을 때 스크롤 위치가 따라가지 않는" React Native ScrollView의 일반적인 동작이 원인이었다.
- **수정**: `ScrollView`에 `ref`를 달고, `isRegenerating`이 `true`로 바뀌는 시점에 `scrollViewRef.current?.scrollToEnd({ animated: true })`를 호출하는 `useEffect`를 추가했다 — 로딩 블록이 나타날 때마다 항상 화면을 새 콘텐츠 맨 아래까지 자동으로 스크롤해서, 이미 반영돼 있던 안전영역 패딩이 실제로 보이는 영역 안에 들어오게 했다.
- **검증**: 이번 세션은 이 프로젝트 원격 브릿지가 파일 편집·전달까지만 되고 `npx tsc --noEmit`/`npx eslint .`를 직접 실행할 수 있는 셸 마운트는 이번엔 연결되지 않았다 — 사용자 로컬 터미널에서 두 명령과 실기기 확인(로딩 중 상태가 기기 하단에서 잘리지 않는지) 부탁드린다.

### 121. 선호번호 · 제외번호 세트 관리 — 저장 확인 안 됨 + 제외번호 세트 카드 가독성
- **피드백**: (1) "내 번호" 탭에서 선호번호를 지정해도 화면에 아무 표시가 없어서 반영이 안 된 줄 알았는데, "선호번호 · 제외번호 세트 관리" 화면에 직접 들어가 보면 번호가 정상적으로 선택돼 있었다. (2) 저장된 제외번호 세트의 "삭제" 버튼이 있긴 한데 가독성이 떨어지고, 제외번호 세트 목록 전반의 정리가 필요하다. 스크린샷 5장(내 번호 탭 빈 화면, 선호번호·제외번호 세트 화면 2장, 삭제 버튼 위치를 빨간 박스로 표시한 스크린샷 2장)으로 확인.
- **원인 1**: `app/(tabs)/tickets.tsx`의 빈 상태 문구("아직 저장한 번호가 없습니다")는 저장된 티켓(생성해서 저장한 번호 게임) 유무만 가리킬 뿐, 선호번호·제외번호 세트와는 무관하다. 그런데 이 탭에는 "선호번호 · 제외번호 세트 관리"로 가는 링크만 있고 현재 몇 개가 설정돼 있는지 보여주는 요약이 전혀 없었다 — 실제로는 저장이 정상 작동했지만(`app/preferences.tsx`가 `updatePreferences`로 AsyncStorage에 올바르게 저장), 진입점(내 번호 탭)에서 그 사실을 확인할 방법이 없어 "반영이 안 됐다"는 오해를 유발했다.
- **원인 2**: `app/preferences.tsx`의 제외번호 세트 카드는 삭제를 `fontSize: 11, color: colors.textMuted`짜리 일반 텍스트로만 표시해 버튼이라는 인지가 잘 안 됐고(터치 영역도 텍스트 크기만큼), 확인 절차 없이 눌리는 즉시 삭제됐다(이 앱의 다른 삭제 동작들 — 예: 내 번호 탭의 티켓 삭제 — 은 전부 확인 Alert를 거치는 것과 비일관). 제외번호 목록도 `set.numbers.join(", ")`로 숫자를 쉼표로만 나열해, 앱 다른 화면이 번호를 항상 색깔 있는 로또 공으로 보여주는 것과 다른 표현 방식이었다.
- **수정**:
  - `app/(tabs)/tickets.tsx` — `getPreferences`/`getExclusionSets`를 `useFocusEffect`에 같이 걸어(탭에 다시 들어올 때마다 갱신) 선호번호 개수·제외번호 세트 개수를 불러오고, "선호번호 · 제외번호 세트 관리" 링크 바로 아래에 "선호번호 N개 · 제외번호 세트 N개 설정됨"을 보여준다(둘 다 0개면 표시하지 않음). 빈 상태 화면과 티켓 목록 상단 헤더 두 곳 모두에 적용해, 어느 진입 경로로 오든 하위 화면에 들어가지 않고도 저장이 반영됐는지 바로 확인할 수 있게 했다.
  - `app/preferences.tsx` — 제외번호 세트 카드를 재구성: 세트 이름 옆에 "N개" 배지를 붙이고, 삭제는 빨간 배경 칩 버튼(휴지통 아이콘 + "삭제" 텍스트, `hitSlop` 추가)으로 바꿔 누를 수 있는 버튼임을 명확히 했다. 삭제 버튼을 누르면 이제 "삭제/취소" 확인 Alert을 거친 뒤에만 실제로 삭제되도록 해, 다른 화면의 삭제 동작들과 통일했다. 번호 목록은 쉼표 텍스트 대신 `LottoBall`(size 26)을 나열해 앱 전체와 동일한 방식으로 보여준다.
- **검증**: 이번 세션도 이 프로젝트 원격 브릿지의 셸 마운트가 연결되지 않아 `npx tsc --noEmit`/`npx eslint .`를 직접 실행하지 못했다 — 코드 리뷰 기준으로는 기존 `tints.red`/`colors.surfaceAlt` 등 이미 다른 화면에서 쓰이는 토큰만 재사용했고 새 타입 오류 가능성은 낮다고 본다. 사용자 로컬 터미널에서 두 명령과, 실기기에서 (1) 내 번호 탭에 선호번호/제외번호 세트 요약이 뜨는지, (2) 제외번호 세트 카드가 로또 공으로 보이는지, (3) 삭제 버튼을 누르면 확인 Alert이 뜨는지 확인 부탁드린다.

### 122. 로또 연구소 — "당첨번호 합계 추세" 그래프 좌우 스크롤 인지성 개선
- **피드백**: "당첨번호 합계 추세 (최근 52회)" 그래프가 좌우로 스크롤되는데, 스크롤이 된다는 걸 사용자가 인지할 수 있으면 좋겠다는 요청. 다만 텍스트로 안내하는 방식은 원하지 않음. "하단 막대버튼 같은 것"을 예시로 들며, 타 앱에서 가장 많이 쓰는 방식으로 구현해달라는 요청.
- **배경**: `src/components/SumTrendChart.tsx`는 예전(QA 피드백 반영분)에 이미 한 번 이 문제를 다룬 적이 있다 — 카드와 다른 배경색으로 "여기는 스크롤 영역"임을 표시하고, `showsHorizontalScrollIndicator`를 켜서 네이티브 스크롤바를 노출했었다. 그런데 iOS/Android의 네이티브 스크롤 인디케이터는 손을 대기 전이나 스크롤 중이 아니면 아예 안 보이거나 아주 잠깐만 나타났다 사라지는 방식이라, "화면을 보자마자, 만지지 않고도" 스크롤 가능 여부를 알아채기는 여전히 어려웠다.
- **수정**: `SumTrendChart.tsx`에 차트 밑에 항상 떠 있는(터치 여부와 무관하게 계속 보이는) 커스텀 스크롤 위치 막대를 추가했다 — 얇은 트랙(전체 스크롤 가능 폭)과 그 위의 막대(thumb, 지금 보이는 비율만큼의 너비)로 구성되며, 막대의 길이로 "전체 중 지금 보이는 비율"을, 막대의 위치로 "지금 어디를 보고 있는지"를 문구·아이콘 없이 바로 전달한다(주식·헬스 앱 등에서 흔히 쓰는 스크롤 위치 표시 방식). `onLayout`으로 뷰포트 폭을, `onScroll`(`Animated.event`, `scrollEventThrottle=16`)로 현재 스크롤 위치를 측정해 막대의 너비·위치를 실시간으로 계산한다. 콘텐츠가 뷰포트보다 좁아서 애초에 스크롤할 필요가 없는 경우(회차 데이터가 아주 적을 때)엔 막대 자체를 숨긴다. 네이티브 인디케이터와 중복 노출되지 않도록 `showsHorizontalScrollIndicator`는 다시 껐다.
- **검증**: 이번 세션도 원격 브릿지의 셸 마운트가 연결되지 않아 `npx tsc --noEmit`/`npx eslint .`를 직접 돌리지 못했다 — 새로 쓴 `Animated.event`/`onLayout`/interpolate 패턴은 RN에서 흔한 표준 패턴이라 타입 오류 가능성은 낮다고 보지만, 사용자 로컬 터미널에서 두 명령과 실기기에서 (1) 스크롤 안 한 상태에서도 막대가 바로 보이는지, (2) 스크롤할 때 막대 위치가 자연스럽게 따라오는지, (3) 처음 진입 시 최신 회차(오른쪽 끝)로 스크롤된 상태에서 막대도 오른쪽 끝에 위치하는지 확인 부탁드린다.

### 123. [문서] `DESIGN_GUIDE.md` 신규 작성 — "고급스럽고 세련된" 방향 GPT 초안 선별·확정 (코드 반영 전 단계)
- **배경**: 사용자가 GPT로 정리한 디자인 가이드 초안("Refined Utility × Crafted Delight" — Color/Typography/Spacing/Card/3D Art Direction/CTA/숏컷 메뉴/하단 내비/모션 스펙 등 전 영역 포함)을 붙여넣고, "이 앱이랑 잘 맞는 부분을 선별해서 가이드화하고, 이를 바탕으로 디자인을 더 고급스럽게 반영해달라"고 요청했다. 이 세션은 실기기 화면을 직접 렌더링·스크린샷할 수 없어(코드 편집과 기기 간 파일 전송만 가능) 앱 전체 시각 변경을 눈으로 확인하지 못한 채 진행하는 것은 리스크가 크다고 판단, `AskUserQuestion`으로 범위를 먼저 확인했다 — 사용자는 "우선은 가이드 문서를 만들어보자"를 선택해, 이번 라운드는 **가이드 문서 작성까지만** 진행하고 코드는 건드리지 않기로 확정했다.
- **작업**: GPT 초안을 그대로 옮기지 않고, `src/theme/colors.ts`·`app/(tabs)/index.tsx`·`app/(tabs)/generate.tsx`·`app/(tabs)/_layout.tsx`·`app/_layout.tsx`·`src/constants/lottery.ts` 등 실제 코드를 직접 열어 지금 상태와 항목별로 대조한 뒤 "그대로 채택 / 조정해서 채택 / 보류·거절" 세 갈래로 분류해 `DESIGN_GUIDE.md`(14절 구성: Vision·핵심 원칙 6가지·Visual Language·Color System·Typography·Spacing·Card Design·3D Art Direction·로또 공·CTA·일관성 점검·보류 목록·체크리스트·반영 로드맵)를 새로 작성했다. 코드 대조 과정에서 발견한 구체적 사실들: 브랜드 "블루"가 `#2563EB`/`#496DA3`~`#20385E`(홈 히어로 그라디언트)/`#0F172A`(스플래시·헤더·바로가기 버튼)/`#4338CA`(인디고 틴트) 네 갈래로 흩어져 중앙 토큰이 없음, `generate.tsx` 배지가 추천(파랑)/HOT(빨강)/NEW(보라) 세 가지 색으로 제각각임, 카드 `borderRadius`가 화면마다 8~20px로 흩어져 있음, 하단 탭 아이콘이 코드 주석에도 명시된 "입체(글로시) 풀컬러 PNG"인데 이는 GPT 초안이 명시적으로 피하라고 한 패턴과 정확히 충돌함(가장 확실하고 실행 리스크가 낮은 권고로 표시), Pretendard 등 커스텀 폰트가 `useFonts` 호출도 `assets/fonts/`도 없이 전혀 로드돼 있지 않음.
- **의미(핵심 판단 — 사용자 확인 필요)**: GPT 초안의 브랜드 컬러 제안(Violet `#5B4FF2`)은 **채택하지 않는 쪽으로 가이드에 반영**했다 — 이유는 (1) 블루 계열이 이미 앱 전역 수십 개 화면에 쓰이고 있어 교체 시 마이그레이션 리스크가 큼, (2) 사용자가 참고로 든 토스 자체도 블루 브랜드임, (3) GPT 초안 스스로 "카지노/게임 느낌의 보라 그라디언트를 피하라"고 해놓고 브랜드 색을 보라 계열로 제안하는 내부 모순이 있음. 대신 기존 블루 계열을 `brand.primary #2563EB`/`brand.pressed #1D4ED8`/`brand.soft #EFF6FF`/`brand.dark #0F172A` 4개 토큰으로 정리·통합하는 안을 제안했다 — 가이드 문서 안에 "제안이지 확정 아님, 실제 반영 전 사용자 확인 필요"로 명시해뒀다. 로또 공 색상(공식 5구간 색)은 인지 정확도(실제 로또 공과의 일치)가 미감보다 중요하다고 보아 채도를 낮추자는 GPT 제안도 반영하지 않았다.
- **한계**: 실제 3D 에셋 제작(로또 공 재질감, 숏컷 아이콘 리터칭)은 이 세션이 할 수 없는 작업이라 "제작 시 참고 규격"으로만 가이드에 남겼고, Pretendard 폰트 적용은 기술적으로는 가능하지만 이번 라운드 범위 밖으로 분리했다(폰트 파일 확보·`useFonts`·화면별 `fontFamily` 적용이 필요한 별도 작업). 이 문서 자체는 아직 어떤 코드도 바꾸지 않았다 — 사용자가 문서를 검토하고 특히 위 "브랜드 컬러 유지" 판단을 확인해준 뒤, 문서 14절의 로드맵(1단계 색상 토큰 정리 → 2단계 탭 아이콘 2D 전환[가장 빠르고 효과 큰 항목] → 3단계 카드 radius·그림자 통일 → 4단계 배지·CTA 색상 정리 → 5단계(선택) Pretendard → 6단계(선택) 3D 에셋 재제작) 순서대로 이어서 진행한다.

### 124. [디자인 가이드 Phase 1~4] 색상 토큰 정리 → 탭바 아이콘 2D 전환 → 카드 radius·그림자 통일 → 배지·CTA 색 정리
- **배경**: 123번에서 만든 `DESIGN_GUIDE.md`를 검토한 사용자가 "보라(Violet)로 바꾸지 않고 지금의 파랑 계열을 하나로 정리"·"원색(로또공)은 유지, 재질감만 추후 3D 규격 반영, 3D 볼 구현은 1차 디자인 정리 이후로 연기"를 확정하고, 가이드 14절 로드맵의 Phase 1~4를 "제안한 기준으로 진행"하되 "문제가 발생하면 바로 롤백할 수 있어야 한다"는 조건을 명시했다. 이에 따라 Phase 1~4를 각각 **독립적으로 전달·커밋 가능한 단위**로 나눠 순서대로 진행했다(아래 4개 커밋으로 분리해 올리면 특정 단계만 개별 `git revert` 가능).
- **Phase 1 — 색상 토큰 정리**: `src/theme/colors.ts`에 테마 무관 고정 `BrandTokens`(`brand.primary #2563EB`/`brand.primaryPressed #1D4ED8`/`brand.dark #0F172A`)와, 칩·배지용 라이트/다크 쌍 `tints.brand`(라이트 `#EFF6FF`/`#2563EB`, 다크 `#1E3A8A`/`#BFDBFE`)를 새로 추가했다. `src/theme/ThemeContext.tsx`의 `useAppTheme()`에도 `brand`를 같이 노출해 `const { colors, tints, brand } = useAppTheme()`로 한 번에 꺼내 쓸 수 있게 했다. 이 커밋은 **토큰 정의만 추가**하고 기존 화면의 하드코딩 값은 건드리지 않아 회귀 위험이 없다.
- **Phase 2 — 탭바 아이콘 2D 전환**: `app/(tabs)/_layout.tsx`의 "입체(글로시) 풀컬러 PNG" 탭 아이콘 4개(`assets/tab-icons/*.png`, 홈 라이트/다크 반전 버전 포함)를 전부 제거하고, 이미 앱 전역에서 쓰고 있는 `@expo/vector-icons`(Ionicons)로 교체했다 — 홈 `home-outline/home`, 번호 만들기 `color-wand-outline/color-wand`, 로또 연구소 `flask-outline/flask`, 내 번호 `ticket-outline/ticket`(비활성/활성 상태를 outline↔filled로 구분). `tabBarActiveTintColor`를 하드코딩 `"#2563EB"`에서 `brand.primary`로 교체(값은 동일). 기존 홈 아이콘의 다크모드 반전 로직(대비 문제 대응용)도 색을 테마별 tint로 그리는 Ionicons로 바뀌며 자연히 불필요해져 함께 정리했다.
- **Phase 3 — 카드 radius·그림자 통일**: `DESIGN_GUIDE.md` 7절의 2단계 규칙(주요 콘텐츠 카드 20px / 칩·배지·작은 링크 카드 12px)에 맞춰: `app/(tabs)/index.tsx`의 "내가 자주 선택한 번호" 카드류(`card`) 16→20, `app/(tabs)/lab.tsx`의 `card`/`officialCard`/`weeklyCard` 16→20, `app/(tabs)/tickets.tsx`의 `card`(개별 번호 카드) 16→20으로 올리고, `app/(tabs)/index.tsx`의 `shortcutButton`·`app/(tabs)/lab.tsx`의 `retryButton`·`app/(tabs)/tickets.tsx`의 `prefLinkButton`·`src/components/DisclaimerCard.tsx`(앱 전역 안내 문구 카드, 6개 화면에서 공용)를 10→12로 낮췄다. `app/(tabs)/generate.tsx`의 카드 그림자만 원안 대비 소폭 진했던 값(`shadowOpacity 0.06/shadowRadius 10/offset height 4`)을 `0.05/8/3`으로 살짝 절제했다. `officialCard`의 강조용 그림자(0.18/10/4, `#2563EB`)는 의도된 하이라이트 장치라 이번 절제 대상에서 제외했다.
- **Phase 4 — 배지·CTA 색 정리**: `app/(tabs)/index.tsx`의 홈 히어로 "AI로 번호 만들기" CTA — 그라디언트를 `#496DA3→#20385E`에서 `brand.primary→brand.dark`로 토큰화(색감은 기존 남색 계열 유지), 버튼 높이를 `paddingVertical 16→18`(≈48~50px→56px)로 키우고, `brand.primary` 기준 은은한 그림자(전용 바깥 래퍼 `ctaButtonShadow`로 분리 — `overflow:hidden`인 기존 래퍼 안에 두면 그림자가 잘려서 별도로 뺐다)를 새로 추가했으며, Sparkle 아이콘(`Ionicons name="sparkles"`)을 텍스트 앞에 추가했다(화살표는 가이드에서 선택사항으로 명시해 이번엔 생략). `app/(tabs)/generate.tsx`의 추천/HOT/NEW 배지는 "서로 다른 정보라 색 구분 자체는 유지"하되, 각자 따로 박혀 있던 hex(`#2563EB`/`#DC2626`/`#6C5CE7`)를 `brand.primary`/`tints.red.fg`/`tints.purple.fg` 토큰 참조로 바꿔 "정해진 팔레트 안에서 고른 색"이 되도록 정리했다(NEW의 보라만 `#6C5CE7`→`#5B21B6`로 미세 조정 — 다른 화면 보라 배지와 동일한 값).
- **검증**: 이번 세션도 원격 브릿지의 셸 마운트가 연결되지 않아(`sandbox-helper: no Plan9 drive shares mounted`) `npx tsc --noEmit`/`npx eslint .`를 직접 실행하지 못했다 — 4단계 모두 기존에 이미 쓰이는 타입(`AppColors`/`AppTints`/`BrandTokens`)과 이미 의존성에 있는 `@expo/vector-icons`만 사용해 새 타입 오류 가능성은 낮다고 보지만, 사용자 로컬 터미널에서 두 명령 확인이 꼭 필요하다. 실기기에서는 (1) 하단 탭 4개 아이콘이 전부 Ionicons 선 아이콘으로 바뀌고 눌린 탭만 파랗게 채워지는지, (2) 홈/로또 연구소/내 번호 화면의 카드 모서리가 이전보다 약간 더 둥글어졌는지(20px), (3) 홈 히어로 CTA 버튼이 더 커지고 그림자·스파클 아이콘이 보이는지, (4) "번호 만들기" 탭의 추천/HOT/NEW 배지가 색은 유지한 채 잘 보이는지 확인 부탁드린다.
- **롤백 안내**: 사용자가 밝힌 대로, Phase 1(`colors.ts`/`ThemeContext.tsx`) → Phase 2(`(tabs)/_layout.tsx`) → Phase 3(`index.tsx`/`lab.tsx`/`tickets.tsx`/`generate.tsx`/`DisclaimerCard.tsx`) → Phase 4(`index.tsx`/`generate.tsx`)를 각각 **별도 커밋**으로 올리는 걸 권장한다(예: `git add <해당 파일들> && git commit -m "..."`를 4번 반복). 이렇게 하면 특정 단계에서 실기기 확인 중 문제가 발견됐을 때 `git revert <그 단계 커밋>` 한 번으로 그 단계만 되돌리고 나머지 단계는 그대로 유지할 수 있다. 한 번에 전부 커밋해버리면(예: `git add .` 한 번) 이후 부분 롤백이 어려워지니, 최소한 이번 라운드만큼은 단계별 분할 커밋을 권한다.

### 125. [디자인 가이드 Phase 5a] 로또공 표면 재질감 정리 (원색 유지, 하이라이트·가장자리 셰이딩 추가)
- **배경**: 사용자가 124번 이후 "다음 단계(Pretendard 폰트 또는 3D 로또공 재질감 정리)로 이어가자"고 요청하면서 둘 다 순서대로 진행하기로 하고, 로또공 쪽을 먼저 진행하기로 했다. `DESIGN_GUIDE.md` 9절에서 이미 확정한 방향 그대로: 실제 3D 렌더링 에셋 제작은 이 세션에서 불가능하지만, 공식 로또 색상(원색)은 그대로 두고 "표면이 빛을 받는 방식"만 2D 그라디언트로 근사해 다듬는 건 가능하다.
- **원인/이전 상태**: `src/components/LottoBall.tsx`는 완전 평면 단색 원(`backgroundColor: getBallColor(number)`) + 그림자만 있는 구조였다 — 하이라이트도, 가장자리 음영도 없어 앱 전체에서 가장 자주 보이는(모든 화면의 번호 표시) 요소인데도 "카드"들보다 오히려 입체감이 없었다.
- **수정**: `LottoBall.tsx`를 3겹 구조로 재구성했다. ① `ballShadow`(바깥, 그림자 전용 — `shadowOpacity 0.18/shadowRadius 3/offset height 2`, 기존보다 살짝 또렷하게) ② `ballSurface`(공식 색 배경 + `overflow:hidden`으로 안쪽 그라디언트를 원 밖으로 안 나가게 자름) ③ 그 안에 두 개의 `LinearGradient` — 우하단을 은은하게 눌러주는 가장자리 셰이딩(`transparent→rgba(0,0,0,0.16)`)과 좌상단의 작은 원형 하이라이트(`rgba(255,255,255,0.55)→투명`, 10시 방향 — 가이드 8절의 조명 방향 규칙과 동일). 숫자 텍스트는 그대로 평평하게(엠보싱 없이) 맨 위에 얹어 가이드 9절의 "인쇄된 듯한 숫자" 요건을 유지했다. 색상 자체(`getBallColor()`)는 전혀 건드리지 않았다 — 1~10 노랑/11~20 파랑/21~30 빨강/31~40 회색/41~45 초록 그대로.
- **검증**: 이번에도 원격 브릿지 셸이 막혀 있어 `npx tsc --noEmit`/`npx eslint .`를 직접 실행하지 못했다 — `expo-linear-gradient`는 이미 `index.tsx`/`lab.tsx`/`generate.tsx`에서 쓰고 있는 검증된 의존성이라 새 타입 오류 가능성은 낮다고 본다. `LottoBall`은 앱에서 가장 많이 재사용되는 컴포넌트라(티켓 카드, 생성 결과, 로또 연구소 통계, 주사위 등 거의 모든 화면) 사용자 로컬 `tsc`/`eslint` 확인과 함께, 실기기에서 (1) 공 색상 자체는 이전과 동일한지, (2) 좌상단에 은은한 하이라이트가 보이는지, (3) 한 화면에 공이 많이 뜨는 곳(45면체 주사위, 로또 연구소 빈도 카드 등)에서 스크롤이 버벅이지 않는지(그라디언트 레이어가 공 하나당 2개씩 추가됐다) 꼭 확인 부탁드린다 — 버벅임이 느껴지면 바로 알려달라, 그라디언트를 걷어내고 이전 평면 버전으로 되돌릴 수 있다.

### 126. [디자인 가이드 Phase 5b] Pretendard 폰트 도입 (제목·버튼 계층 1차 적용)
- **배경**: 125번에 이어 사용자가 요청한 두 번째 항목. `DESIGN_GUIDE.md` 5절에서 "기술적으로는 가능하지만 폰트 파일 확보·`useFonts`·화면별 `fontFamily` 적용이 필요한 별도 작업"으로 분리해뒀던 Pretendard 폰트를 실제로 도입했다.
- **폰트 파일 확보**: npm 레지스트리에서 `pretendard@1.3.9` 패키지를 받아(이 세션은 임의 CDN 접근이 막혀 있어 jsDelivr 직접 다운로드는 실패, npm 레지스트리는 허용돼 있어 이 경로로 우회) `dist/public/static/`의 표준 4종(Regular/Medium/SemiBold/Bold, `.otf`)을 추출해 `assets/fonts/`에 새로 추가했다. Pretendard는 SIL Open Font License 1.1(상업적 사용·재배포 가능, 폰트 파일과 함께 라이선스 고지 필요)이라 `assets/fonts/Pretendard-LICENSE.txt`도 같이 넣었다. `app.json`은 이미 `expo-font` 플러그인과 `assetBundlePatterns: ["**/*"]`가 설정돼 있어(전 세션부터 미사용 상태로 존재) 추가 설정 없이 바로 번들된다.
- **로딩 인프라**: `app/_layout.tsx`에 `useFonts`로 4종을 로드하고, 폰트 로딩이 끝나기 전에는(`fontsLoaded`도 `fontError`도 아닌 상태) 화면을 아예 그리지 않도록(`return null`) 바꿨다 — 스플래시를 먼저 내리고 화면을 그리기 시작하면 시스템 기본 폰트로 잠깐 렌더링됐다가 Pretendard로 바뀌는 깜빡임(FOUC)이 생기기 때문. 폰트 로딩이 실패해도(`fontError`) 앱이 영원히 멈추지 않도록 이 경우도 "준비 완료"로 취급해 시스템 폰트로 자연스럽게 폴백한다. `src/theme/typography.ts`를 새로 만들어 `fontFamily.regular/medium/semiBold/bold` 상수를 한 곳에서 관리하고 `src/theme/index.ts`에서 같이 export했다.
- **왜 전역 일괄 적용이 아니라 화면별 개별 지정인지**: 웹과 달리 RN은 하나의 `fontFamily`에 여러 굵기를 자동 매핑해주지 않고, 이 앱은 React 19 + RN 0.81 조합이라 `Text.defaultProps`로 전역 기본 폰트를 주입하는 예전 트릭(함수 컴포넌트의 `defaultProps`는 React 19에서 지원이 빠졌다)도 신뢰할 수 없다고 판단해 채택하지 않았다. 대신 각 텍스트 스타일이 `fontFamily.*` 상수를 명시적으로 참조하는 방식을 썼다 — 안전하지만 화면마다 손으로 지정해야 하는 만큼, 이번 1차 반영은 가이드 5절의 상위 타이포 계층(페이지 제목·섹션 제목·카드 제목·CTA/버튼 텍스트)까지만 범위를 좁혔다: `index.tsx`(`heroTitle`/`ctaButtonText`/`cardTitle`/`shortcutButtonText`), `generate.tsx`(`header`/`cardTitle`), `lab.tsx`(`header`/`cardTitle`/`weeklyTitle`), `tickets.tsx`(`emptyText`/`sectionHeaderTitle`) — 전부 `fontWeight`가 이미 600 이상이던 "제목/버튼"급 텍스트만 골라 `fontFamily: fontFamily.bold`를 추가했다. 배지·캡션·안내 문구 같은 12px 이하 하위 텍스트는 이번엔 시스템 폰트 그대로 남겨뒀다 — 실기기에서 상위 계층 결과를 먼저 확인한 뒤, 필요하면 다음 라운드에서 범위를 넓힌다.
- **검증**: 이번에도 원격 브릿지 셸이 막혀 `npx tsc --noEmit`/`npx eslint .`를 직접 실행하지 못했다 — `fontFamily` 상수 참조만 추가한 단순한 변경이라 타입 오류 가능성은 낮다고 보지만, 폰트 로딩 자체는 코드만으로는 100% 보장할 수 없는 영역(파일 손상, 폰트 이름 불일치 등)이라 사용자 로컬 `tsc`/`eslint` 확인과 실기기 확인이 특히 중요하다. 실기기에서 (1) 앱 시작 시 스플래시가 부자연스럽게 오래 걸리지 않는지, (2) 홈/번호 만들기/로또 연구소/내 번호 탭의 제목·카드 제목·CTA 버튼 글자체가 이전과 달라 보이는지(Pretendard 특유의 좀 더 정돈된 느낌), (3) 숫자나 영문이 깨지거나 네모(□)로 나오는 폰트 로딩 실패 징후가 없는지 확인 부탁드린다. 문제가 있으면 `app/_layout.tsx`의 `useFonts` 블록과 4개 화면의 `fontFamily.bold` 지정 줄만 되돌리면 이전 시스템 폰트 상태로 복구된다.

### 127. [디자인 가이드 Phase 5c] "번호 만들기" 하위 10개 화면 + Phase 1~4에서 빠졌던 공유 컴포넌트 3개로 규칙 확장
- **배경**: 125·126번(로또공 재질감·Pretendard)까지 반영한 뒤 사용자가 "반영 안된 이미지나 텍스트 있는지 체크해달라"고 요청했고, 점검 결과 `app/generate/` 아래 10개 화면 전부가 124번 Phase 1~4 롤아웃에서 완전히 빠져 있었다(파편화된 `borderRadius`, 하드코딩 hex, 이전에 발견 못 한 "다섯 번째 파랑" `#3B82F6` 등). 사용자는 "체계를 잡고 가는 게 중요하다"며 10개 화면 전부를 같은 3가지 기준(radius 12/20 2단계, brand 토큰, Pretendard 제목)으로 확장하고, 빠진 파일·이미지가 없는지 반드시 다시 체크해달라고 명시적으로 요청했다.
- **작업 1 — 화면 10개**: `exclusion.tsx`/`ai-search.tsx`/`lucky.tsx`/`dice.tsx`/`destiny.tsx`/`qr-check.tsx`/`result.tsx`/`deep-pattern.tsx`/`deep-pattern-result.tsx`/`deep-pattern-detail.tsx` 전부에 `useAppTheme()`에서 `brand`를 같이 꺼내 쓰도록 확장하고: (1) 카드류(리스트 카드·결과 카드·비주얼 카드) radius를 16~18→20으로, 버튼·인풋류를 10~14→12로, 칩·배지류를 6~8→12로 통일(진행률 바 채움·점·완전히 둥근 pill 배지·원형 공 자리처럼 "기능·장식용 둥근 모양"은 규칙 대상에서 제외해 그대로 뒀다). (2) 그동안 화면마다 흩어져 있던 `#2563EB`/`#1D4ED8`/`#0F172A` 및 `ai-search.tsx`에서 새로 찾은 "다섯 번째 파랑" `#3B82F6`을 전부 `brand.primary`/`brand.primaryPressed`/`brand.dark` 토큰으로 교체. (3) 섹션 제목·페이지 제목·버튼 텍스트·진행률 표시처럼 "제목/버튼급" 텍스트에 `fontFamily: fontFamily.bold` 추가(칩·캡션 등 하위 텍스트는 125·126번과 동일 기준으로 이번에도 범위 밖에 뒀다). 10개 화면의 네이티브 헤더 타이틀은 화면마다 따로 손대지 않고 `app/generate/_layout.tsx`(전체 10개 화면의 공용 `Stack screenOptions`)와 `app/_layout.tsx`(선호번호·개인정보처리방침 등 최상위 화면) 두 곳의 `headerTitleStyle`만 고쳐 한 번에 적용했다 — Phase 5b 때 했던 "공용 지점 하나만 고치기" 패턴을 그대로 재사용한 것이다.
- **작업 2 — 빠진 파일 체크에서 발견한 것(공유 컴포넌트 3개)**: "빠진 파일 없는지 필히 체크"라는 요청에 따라 10개 화면이 실제로 쓰는 공유 컴포넌트(`BottomActionBar`/`NumberGrid`/`DisclaimerCard`/`GeneratedGameCard`/`LottoBallLoader`/`ProbabilityCard`/`LottoBall`/`Dice45`/`deepPattern/*`)를 전부 열어 대조한 결과, `DisclaimerCard`(Phase 3에서 이미 12px로 반영됨, 정상)를 빼고 **`BottomActionBar.tsx`/`GeneratedGameCard.tsx`/`ProbabilityCard.tsx` 3개가 Phase 1~4 어디에도 포함되지 않고 완전히 누락돼 있었다** — 이 세 컴포넌트는 번호 만들기뿐 아니라 홈·로또 연구소 등 앱 전역에서 재사용되는 핵심 컴포넌트라, 이번 라운드에서 함께 정리했다: `BottomActionBar`(하단 고정 액션 버튼, 거의 모든 생성 화면에서 씀) — 기본 색 `"#2563EB"`→`brand.primary`, radius 14→12, 버튼 텍스트에 Pretendard 추가. `GeneratedGameCard`(생성 결과 카드) — radius 16→20, 칩·배지 8→12, 적합도 점수 색 `"#2563EB"`→`brand.primary`. `ProbabilityCard`(확률 요약, 결과 화면 상단) — 원래 `useAppTheme`조차 import하지 않고 있었는데 `brand` 상수만 새로 import해 배경 `"#0F172A"`→`brand.dark`, radius 14→20으로 정리(이 카드는 테마 무관 고정 다크 톤이라 `useAppTheme()` 훅 자체는 여전히 불필요, `brand`만 모듈 상수로 직접 import).
- **의미(사용자 확인 필요) — 보라 3계열 미통일 판단**: 이번 점검 중 앱에 서로 다른 "보라"가 최소 3곳에 독립적으로 쓰이고 있음을 확인했다 — ① `destiny.tsx`(운명의 신) 고유 강조색 `#7C3AED`, ② 딥 패턴 3화면(`deep-pattern`/`-result`/`-detail`) 고유 강조색 `#6C5CE7`, ③ NEW 배지 등에 쓰는 `tints.purple`(라이트 `#5B21B6`/다크 `#DDD6FE`). 123번에서 "블루 계열 통합"을 확정할 때 감사 대상은 `generate.tsx`(허브 화면)의 배지 3색 혼재 문제 하나였고, 이 보라 3계열은 그 감사 범위 밖이었다 — 이번 라운드에서도 "화면 각각의 고유 정체성"으로 보아 통합하지 않고 그대로 뒀다(코드에 그 판단 근거를 주석으로 남겨뒀다). 다만 파랑을 하나로 정리한 것과 같은 기준을 보라에도 적용할지는 사용자 확인이 필요한 별도 판단이라 여기서 명시적으로 알려드린다 — 통합을 원하면 다음 라운드에서 진행 가능하다.
- **의미 — 이번 라운드에서 의도적으로 손대지 않은 부분**: `#2563EB`/`#0F172A` 계열 하드코딩이 아직 남아 있는 곳이 이 저장소에 더 있다 — `app/(tabs)/index.tsx`·`tickets.tsx`·`lab.tsx`·`generate.tsx`(허브 화면 4개, 124번에서 카드 radius는 이미 정리했지만 그 외 산발적 hex는 남아 있음), `app/preferences.tsx`, `src/components/ErrorBoundary.tsx`, `app.json`의 스플래시 배경색, `legal/privacy-policy.html`. 사용자 요청이 명시적으로 "10개 화면까지"였고 이 파일들은 그 10개 화면·그 화면들이 직접 쓰는 컴포넌트가 아니라서 이번 라운드 범위 밖으로 남겨뒀다 — 다음 라운드에서 이 나머지도 `brand.*` 토큰으로 정리할지는 별도로 여쭤보고 진행하는 게 맞다고 판단했다.
- **검증**: 이번 세션도 원격 브릿지 셸 마운트가 막혀(`sandbox-helper: no Plan9 drive shares mounted`) `npx tsc --noEmit`/`npx eslint .`를 직접 실행하지 못했다 — 10개 화면 + 공유 컴포넌트 3개 모두 이미 검증된 타입(`AppColors`/`AppTints`/`BrandTokens`)과 기존에 쓰이던 `fontFamily`/`brand` export만 재사용해 새 타입 오류 가능성은 낮다고 보지만, 이번 라운드는 지금까지 중 변경 파일 수가 가장 많아(화면 10개 + 레이아웃 2개 + 공유 컴포넌트 3개) 사용자 로컬 `tsc`/`eslint` 확인이 특히 중요하다. 사용자가 예고한 대로, 다음은 실기기에서 10개 화면을 직접 돌아보며 (1) 카드·버튼 모서리가 12/20px 기준으로 고르게 보이는지, (2) 제목·버튼 글자가 Pretendard로 바뀌었는지, (3) 브랜드 블루가 전부 같은 톤으로 보이는지(이질감 있는 파랑이 남아 있지 않은지), (4) 운명의 신·딥 패턴 화면의 보라가 의도대로 그대로인지 확인해주시면 된다.
- **롤백 안내**: 이번 라운드는 화면 10개 + 레이아웃 2개 + 공유 컴포넌트 3개로 파일 수가 많아, 아래처럼 두 커밋으로 나눠 올리는 걸 권한다 — (1) 화면 10개 + 레이아웃 2개, (2) 공유 컴포넌트 3개(`BottomActionBar`/`GeneratedGameCard`/`ProbabilityCard`). 공유 컴포넌트는 화면보다 영향 범위가 넓으니(다른 탭에도 쓰임) 문제가 생기면 이 커밋만 따로 `git revert`할 수 있게 분리했다.

### 128. [안전영역 재점검] 하단 고정 버튼·토스트가 기기 시스템 바에 가려질 수 있던 지점 3곳 수정
- **배경**: 127번 커밋·푸시 직후 사용자가 "화면이 밀리거나 디바이스 버튼에 가려지지 않는지 다시 체크해봐, 상하단 화면에 제대로 안 들어오는 것도 이상하니까 정확히 체크해야 한다"고 재확인을 요청했다. 76번에서 이미 한 번 "하단 고정 버튼이 기기 내비게이션 바와 겹치는" 같은 종류의 문제를 딥 패턴 상세 화면에서 고친 전례가 있었고, 그때 `deep-pattern-result.tsx`/`qr-check.tsx`는 "버튼이 스크롤 콘텐츠 안에 있어 같은 위험이 없다"고 결론 내렸었다 — 이번엔 그 판단을 다시 검증했다.
- **점검 방법**: `app/generate/` 10개 화면 전체에서 (1) `position: "absolute"`로 화면 하단/상단에 고정된 요소, (2) 하단 고정 버튼 바(`BottomActionBar`) 없이 스크롤 콘텐츠 맨 아래에 주요 버튼이 놓인 화면을 전부 찾아 하나하나 확인했다. `app.json`의 `androidNavigationBar` 설정과 이 프로젝트의 Expo SDK(54)/RN(0.81) 조합은 안드로이드에서 엣지투엣지 렌더링이 기본(Android 15 요건)이라, 시스템 제스처 바가 화면 위에 반투명하게 얹히고 그 아래 콘텐츠가 실제로 그 뒤까지 그려질 수 있다는 점을 확인 기준으로 삼았다 — 즉 고정 px 여백만으로는 기기에 따라 부족할 수 있다.
- **발견 1 — `exclusion.tsx`의 저장 완료 토스트**: "세트 저장" 후 잠깐 뜨는 토스트(`toast` 스타일)가 화면(container) 기준 `bottom: 92` 고정값이었다. 반면 바로 아래 `BottomActionBar`는 기기 안전영역(`insets.bottom`)만큼 자기 높이를 늘리는데, 토스트는 그 변화를 반영하지 못해 안전영역이 큰 기기(엣지투엣지 안드로이드 제스처 바 등, 대략 insets.bottom 30~50px대)에서는 계산상 토스트 아랫부분이 버튼 바 상단과 겹치는 지점까지 내려온다 — 76번이 다룬 "고정 버튼 자체가 겹치는" 문제와는 다른, 이번에 처음 발견한 지점이다. 76번 재발방지 점검은 "고정 버튼 바"만 훑었고 이런 임시 토스트 알림까지는 포함하지 않았었다.
- **발견 2 — `deep-pattern-result.tsx`/`qr-check.tsx`(결과 화면)의 스크롤 하단 버튼**: 76번 당시엔 "버튼이 스크롤 콘텐츠 안에 있어 겹칠 위험이 없다"고 결론 냈는데, 이는 "스크롤해서 볼 수는 있다"는 뜻이지 "맨 아래까지 스크롤했을 때 시스템 바보다 위로 완전히 벗어난다"는 뜻은 아니었다 — `paddingBottom`이 각각 고정 24px/32px뿐이라 엣지투엣지 안드로이드 기기의 제스처 바 높이보다 작으면, 스크롤을 끝까지 내려도 "다시 생성/N게임 모두 저장"(`deep-pattern-result.tsx`)이나 "결과 공유/다시 스캔"(`qr-check.tsx`) 버튼이 시스템 바에 일부 가려진 채로 스크롤이 끝나버릴 수 있다. 76번의 결론이 이 조건(엣지투엣지 항상 적용)까지 감안하지 못했던 셈이다.
- **수정**: 세 파일 모두 `useSafeAreaInsets()`를 새로 가져와, `exclusion.tsx`는 토스트의 `bottom`을 `92 + insets.bottom`으로(인라인 스타일로 분리, 기본 스타일 객체에서는 `bottom` 값 제거), `deep-pattern-result.tsx`/`qr-check.tsx`는 각 `ScrollView`의 `contentContainerStyle.paddingBottom`을 `insets.bottom + 24`/`insets.bottom + 32`로 바꿨다 — `result.tsx`가 이미 쓰고 있던 것과 동일한 패턴(`insets.bottom + 24`)이다.
- **함께 확인하고 손대지 않은 것**: (1) 나머지 7개 화면(`ai-search`/`lucky`/`dice`/`destiny`/`deep-pattern`/`deep-pattern-detail`/`result`)은 전부 `BottomActionBar`(자체적으로 `insets.bottom + 12` 처리)를 쓰거나(`result.tsx`) 이미 `insets.bottom`을 반영하고 있어 추가 수정이 필요 없음을 코드로 재확인했다. (2) `qr-check.tsx`의 카메라 스캔 화면에 있는 `closeButton`(`top: 56`)은 네이티브 헤더(이 화면도 `generate/_layout.tsx`의 공용 헤더를 그대로 쓴다)가 이미 켜져 있어 상태표시줄 아래에 위치하므로 안전영역과는 무관하다 — 다만 헤더의 기본 뒤로가기 버튼과 이 커스텀 "닫기" 버튼이 사실상 중복 기능이라는 점은 발견했다(레이아웃 겹침 문제는 아니라 이번엔 그대로 뒀다). 이 UX 중복을 정리하길 원하면 별도로 진행 가능하다.
- **검증**: 이번에도 원격 브릿지 셸이 막혀(`sandbox-helper: no Plan9 drive shares mounted`) `npx tsc --noEmit`/`npx eslint .`를 직접 실행하지 못했다 — `useSafeAreaInsets()`는 이미 `result.tsx`/`BottomActionBar.tsx`/`SettingsSheet.tsx` 등에서 쓰이는 검증된 훅이라 새 타입 오류 가능성은 낮다고 본다. 실기기에서, 특히 하단 제스처 내비게이션을 쓰는 안드로이드 기기에서 (1) 제외하고 생성 화면에서 세트를 저장했을 때 토스트가 하단 버튼과 겹치지 않는지, (2) 딥 패턴 탐색 결과 화면과 QR 당첨 확인 결과 화면에서 맨 아래로 스크롤했을 때 버튼이 시스템 바에 가려지지 않고 완전히 보이는지 확인 부탁드린다.

### 129. [디자인 가이드 Phase 5c] 보라 3계열 통일(`accentViolet` 신설) + 브랜드 토큰 정리를 탭 허브·설정·에러 화면·정적 페이지까지 확장
- **배경**: 127번에서 "사용자 확인 필요"로 명시적으로 남겨뒀던 두 가지 열린 판단 — ① 보라 3계열(`destiny.tsx` `#7C3AED`, 딥 패턴 3화면 `#6C5CE7`, `tints.purple`) 통일 여부, ② `brand.*` 토큰 정리를 탭 허브 4개·`preferences.tsx`·`ErrorBoundary.tsx`·`app.json`·`legal/privacy-policy.html`까지 확장할지 — 에 대해 사용자가 이번엔 둘 다 "통일/확장하자"고 명시적으로 승인했고, "적용 후 제대로 되었는지 필히 체크해달라"고 요청했다.
- **작업 1 — 보라 통일 판단 및 `accentViolet` 신설**: `destiny.tsx`의 기존 주석이 이미 "이 밝은 톤(`#C4B5FD`)은 lab.tsx의 freqCountLight와 같은 값"이라고 명시하고 있었다 — 즉 `#7C3AED`/`#C4B5FD` 조합이 사실상 두 화면에 걸쳐 쓰이던 공용 값이었다. Design Principle(§15 ④ "다른 화면과 같은 앱처럼 보이는가") 기준으로, 같은 "번호 만들기" 탭 하위의 두 자매 기능(운명의 신/딥 패턴)이 서로 다른 보라를 쓰는 건 미묘한 이질감을 준다고 판단해 `#6C5CE7`/`#C9C2FF` 쪽을 버리고 `#7C3AED`/`#C4B5FD` 쪽으로 통일했다. `src/theme/colors.ts`에 `brand`와 나란히 `accentViolet: { primary, light }` 토큰을 새로 선언(판단 근거를 주석으로 남김)하고, `destiny.tsx`/`deep-pattern.tsx`/`deep-pattern-result.tsx`/`deep-pattern-detail.tsx`/`lab.tsx`(freqCountLight)뿐 아니라, 처음 점검에서는 빠져 있던 `src/components/deepPattern/` 하위 6개 시각화 서브컴포넌트(`DeepPatternIcon`/`DeepPatternLoadingBoard`/`PatternBoard`/`PatternThumb`/`PatternPositionMap`/`PatternMixSlider` — SVG 아이콘·로딩 애니메이션·패턴 보드 선/점·슬라이더 등)에 남아 있던 `#6C5CE7`/`#C9C2FF` 리터럴까지 전부 `accentViolet.primary`/`.light`로 교체했다. `tints.purple`(NEW 배지 등 옅은 배경 배지용, 라이트/다크로 뒤집힘)은 CTA·아이콘용 `accentViolet`(테마 무관 고정)과 역할이 다르다고 판단해(마치 `brand.primary`와 `tints.brand`가 나뉘어 있듯) 통일 대상에서 제외하고 그대로 뒀다 — `generate.tsx`의 NEW 배지가 이미 이 구분을 문서화해뒀던 것과 일관된 판단이다.
- **작업 2 — 브랜드 토큰 정리 확장**: 127번이 범위 밖으로 남겼던 파일들을 전수 재점검했다. `app/(tabs)/index.tsx`(`shortcutButton` 배경, 이미 `brand`를 받고 있었는데 이 버튼만 하드코딩이 남아 있었음), `app/(tabs)/lab.tsx`(재시도 스피너·공식 카드 테두리/그림자·공식 배지·재시도 버튼 텍스트·이번 주 리포트 카드 6곳 — `createStyles`가 `brand`를 안 받고 있어 시그니처에 추가), `app/(tabs)/tickets.tsx`(편집 중 카드 테두리·회차입력 버튼 2곳·당첨확인 버튼·공유/선호번호 링크 텍스트 6곳 — 마찬가지로 `createStyles`에 `brand` 추가), `app/preferences.tsx`(선호번호 저장 버튼), `src/components/ErrorBoundary.tsx`(재시도 버튼 — `useAppTheme()`는 못 쓰지만 `brand`는 테마 무관 상수라 직접 import), `app/_layout.tsx`/`app/generate/_layout.tsx`(스택 헤더 배경 — 127번에서 `headerTitleStyle`만 고치고 `headerStyle.backgroundColor`는 놓쳤던 부분)까지 전부 `brand.primary`/`brand.dark`로 교체했다. `app/(tabs)/generate.tsx`는 재확인 결과 123번에서 이미 `brand`로 완전히 정리돼 있어 이번엔 손댈 게 없었다. `shadowColor: "#0F172A"`류(순수 중립 그림자, `tickets.tsx`/`generate.tsx`)와 `NumberGrid.tsx`의 `cellTextSelected`(공 배경 위 텍스트색, 이전부터 의도적으로 보존)는 이번에도 대상에서 제외했다 — 전자는 브랜드 표면색이 아니라 그림자 틴트 용도, 후자는 이미 과거 세션에서 검토·보존이 확정된 값이라는 판단을 유지했다.
- **작업 3 — `app.json`/`legal/privacy-policy.html`**: `app.json`의 스플래시 배경(`#0F172A`)은 값 자체가 이미 `brand.dark`와 정확히 일치했다 — 다만 JSON 설정 파일이라 JS 토큰을 import할 방법이 없어(주석도 못 남김) 코드 변경 없이 "일치 확인"만 하고 DESIGN_GUIDE.md 4절에 그 사실을 남겼다. `legal/privacy-policy.html`은 독립 정적 페이지라 마찬가지로 토큰을 직접 쓸 수 없어, `lightColors`와 1:1로 맞춘 CSS 변수(`--bg`/`--text-primary`/`--text-secondary`/`--text-muted`)를 새로 선언해 값의 출처를 문서화하고 각 규칙을 그 변수로 바꿨다 — 이 과정에서 `.intro`만 본문(`p`)과 다른 회색(`#334155`)을 쓰고 있던 실제 불일치를 발견해 `--text-secondary`(`#475569`)로 통일했다(이전엔 발견되지 않았던 사소한 버그).
- **검증**: 코드 레벨로 (1) 수정한 20개 파일 전체에서 중괄호 `{`/`}` 개수가 일치하는지(문법 깨짐 없음) 스크립트로 확인, (2) 대상 hex(`#2563EB`/`#1D4ED8`/`#7C3AED`/`#6C5CE7`/`#C9C2FF`/`#C4B5FD`)가 실제 코드(주석 제외)에 더 남아 있지 않은지 저장소 전체 재검색, (3) `tickets.tsx`/`lab.tsx`/`preferences.tsx`의 `createStyles` 시그니처 변경이 호출부(`useMemo` 의존성 배열 포함)와 정확히 맞물리는지, `tints`처럼 그대로 남겨둔 기존 파라미터가 실제로 계속 쓰이고 있는지(미사용 변수로 안 남는지) 각각 확인했다. 이번에도 원격 브릿지 셸이 막혀(`sandbox-helper: no Plan9 drive shares mounted`) `npx tsc --noEmit`/`npx eslint .`는 실행하지 못했다 — 위 확인들로 정적 오류 가능성은 낮췄지만, 이번 라운드도 변경 파일이 많으니(20개) 사용자 로컬 `tsc`/`eslint` 확인이 중요하다. 실기기 확인 포인트: (1) 운명의 신과 딥 패턴 화면을 오갈 때 보라색 톤이 완전히 같아 보이는지, (2) 홈/번호 만들기/로또 연구소/내 번호 탭과 선호번호 설정 화면의 파란 버튼·강조색이 전부 같은 톤으로 보이는지, (3) 앱을 강제 종료 후 재실행했을 때 스플래시와 화면 헤더 색이 자연스럽게 이어지는지, (4) 렌더링 오류를 인위로 유발하기는 어려우니 이 부분은 코드 리뷰로 갈음(에러 화면 버튼 색만 톤 확인).
- **롤백 안내**: 이번 라운드는 성격이 다른 두 작업(보라 통일 / 브랜드 토큰 확장)이 섞여 있어 두 커밋으로 나누는 걸 권한다 — (1) `src/theme/colors.ts`의 `accentViolet` 신설 + 그 토큰을 쓰는 모든 파일(`destiny.tsx`/`deep-pattern*.tsx`/`lab.tsx`/`deepPattern/` 서브컴포넌트 6개), (2) 나머지 브랜드 토큰 확장 파일(탭 허브 3개·`preferences.tsx`·`ErrorBoundary.tsx`·레이아웃 2개·`privacy-policy.html`) — 문제가 생기면 원인을 좁혀 해당 커밋만 되돌릴 수 있다. `DESIGN_GUIDE.md`는 (2)에 함께 포함해도 무방하다(코드가 아니라 문서 반영이라 리스크가 낮음).

### 130. [실기기 QA 반영] Phase 5a/5b/2 되돌림 — 폰트·로또공 3D 효과 원복 + 탭 아이콘 PNG 복원(회색↔컬러 포커스 전환 신규 추가)
- **배경**: 129번 이후 첫 실기기 빌드(`eas build --platform android --profile preview`)로 실제 기기에서 확인한 결과, Phase 5a(로또공 입체 효과)·Phase 5b(Pretendard 폰트)·Phase 2(탭바 아이콘 2D 선 아이콘 전환) 세 가지가 기대와 다르게 보인다는 피드백을 받았다. 스크린샷 2장(문제 지점 빨간 박스 표시 + 원하는 형태의 참고 이미지)과 함께 요청한 내용: (1) 앱 전체 적용 폰트 원복, (2) 메인 화면 로또공 3D 효과 제거, (3) 하단 탭 아이콘을 원래 PNG로 되돌리되 새 기능 추가 — "미선택 시 회색, 선택 시 탭별 고유색"이 아이콘 자체에도 적용(기존엔 선택 시 하단 텍스트만 파란색으로 바뀌고 아이콘은 고정색이었음). 탭별 색상과 구현 방식(벡터 아이콘 재도입 vs PNG 원본 복원)은 참고 스크린샷만으로는 확정할 수 없어 `AskUserQuestion`으로 확인 — 홈=골드/앰버(기존 로고 톤), 로또 연구소=다크 네이비(`brand.dark` 톤), 내 번호=보라(번호 만들기와 색 겹침 허용), 구현 방식=PNG 원본 복원.
- **작업 1 — 폰트 원복**: `app/_layout.tsx`에서 Pretendard 4종 `useFonts` 로딩과 로딩 게이트(`fontsLoaded`/`fontError`/`fontsReady`, "폰트 준비 전 렌더 안 함")를 제거했다. `src/theme/typography.ts`의 `fontFamily` 토큰과 그걸 참조하는 58곳의 개별 스타일은 그대로 뒀다 — 로딩만 꺼지면 RN이 이름을 못 찾는 커스텀 폰트를 에러 없이 시스템 기본 폰트로 조용히 폴백하기 때문에, 58곳을 일일이 되돌리지 않고도 전체가 시스템 폰트로 보인다. `assets/fonts/*.otf` 파일도 남겨뒀다 — 나중에 재도입하고 싶으면 이 파일의 `useFonts` 호출만 되돌리면 된다.
- **작업 2 — 로또공 3D 효과 원복**: `src/components/LottoBall.tsx`에서 Phase 5a가 추가한 `LinearGradient` 오버레이 2겹(가장자리 셰이딩 + 좌상단 하이라이트)과 3겹 구조(`ballShadow`/`ballSurface`/`ballHighlight`)를 제거하고, 배경색+그림자만 있는 단일 `View` 구조로 되돌렸다. 컴포넌트 자체 주석에 "평면 단색 원(그림자만 있던 이전 버전)"이라고 명시돼 있어 원복 대상이 명확했다. `getBallColor()`로 정해지는 공식 로또 색상 자체는 그대로다.
- **작업 3 — 탭 아이콘 PNG 복원 + 회색/컬러 포커스 전환 신규 추가**: 사용자가 `git checkout 1a3082f -- assets/tab-icons`로 Phase 2 삭제 이전의 원본 PNG 5장(`home.png`/`home-dark.png`/`generate.png`/`lab.png`/`tickets.png`)을 직접 복원·커밋·푸시했다. 이 5장을 스테이징해 실제로 열어본 결과 `lab.png`(로또 연구소)가 참고 스크린샷에서 보였던 것과 달리 실제로는 연보라 그라디언트였다 — 사용자가 명시적으로 요청한 "다크 네이비" 톤과 달라, Pillow의 `ImageOps.colorize()`로 명암(그라디언트 구조)은 유지한 채 색상만 `brand.dark` 계열 네이비로 다시 칠해 `lab.png`를 그 자리에서 교체했다(옛 보라 원본을 안 쓰는 채로 남겨두지 않기 위해 — 과거 세션에서 지적됐던 "죽은 자산" 패턴 재발 방지). `generate.png`/`tickets.png`는 원래도 보라 계열이라 사용자가 승인한 대로 그대로 뒀다. 이어서 5장 전부(교체된 `lab.png` 포함)에서 `ImageOps.grayscale()`로 명암은 유지한 채 채도만 제거한 "회색" 버전 5장(`grey-home.png`/`grey-home-dark.png`/`grey-generate.png`/`grey-lab.png`/`grey-tickets.png`)을 새로 만들었다. `app/(tabs)/_layout.tsx`는 `Ionicons` 의존을 완전히 제거하고, 탭마다 회색/컬러 PNG 쌍을 `Record`로 선언해 `focused` 여부에 따라 `<Image source={...}>`를 교체하는 `TabIcon` 컴포넌트로 다시 작성했다(래스터 이미지는 벡터 아이콘의 `color` prop처럼 런타임 tint를 줄 수 없어, 미리 두 가지 색상 버전을 준비해 교체하는 방식을 썼다). 홈 아이콘만 라이트/다크 테마별 별도 파일(`-dark` 접미사)이 기존부터 필요했던 이유(어두운 탭바 배경 위에서 라이트용 짙은 남색 하우스의 대비 저하)를 그대로 유지해 회색 버전도 테마별로 2장씩 만들었다. `tabBarActiveTintColor`(`brand.primary`)/`tabBarInactiveTintColor`는 손대지 않았다 — 사용자가 설명한 기존 동작(선택 시 하단 텍스트만 파란색)은 그대로 유지하고, 아이콘 회색↔컬러 전환은 그 위에 추가된 기능이다.
- **검증**: `LottoBall.tsx`·`app/_layout.tsx`·`app/(tabs)/_layout.tsx` 전체를 다시 읽어 문법(중괄호 짝)·import 정리(미사용 `Ionicons`/`LinearGradient` import가 안 남았는지, `Ionicons` 문자열이 코드에 실사용으로 안 남았는지 grep 재확인 — 주석 1곳(원복 사유 설명)만 남아 있음을 확인)·`require()` 경로 5쌍 10개가 실제 파일명과 정확히 일치하는지 확인했다. 새로 만든 회색 PNG 5장 + 교체된 `lab.png`는 `Read` 도구로 직접 열어 시각 확인(실루엣이 원본과 동일하게 유지되고, `lab.png`는 자연스러운 네이비 그라디언트로 바뀐 것을 확인). 기기 커밋 후 `assets/tab-icons` 디렉터리를 재조회해 10개 파일(원본 4장 유지 + `lab.png` 교체 + 회색 5장 신규)의 파일 크기가 로컬본과 정확히 일치하는지 확인했다. 이번에도 원격 브릿지 셸이 막혀(`sandbox-helper: no Plan9 drive shares mounted`) `npx tsc --noEmit`/`npx eslint .`는 실행하지 못했다 — 특히 이번엔 `require()` 경로 오타는 `tsc`/`eslint`로는 안 잡히고 번들러(Metro) 단계에서만 드러나므로, 사용자가 로컬에서 `npx expo start`(또는 다음 EAS 빌드)를 한 번 돌려 자산 경로 에러가 없는지 반드시 확인해야 한다. 실기기 확인 포인트: (1) 앱 전체 글꼴이 시스템 기본 폰트로 보이는지, (2) 메인 화면 로또공이 평면 단색으로 보이는지, (3) 하단 탭 4개 모두 미선택 시 회색 아이콘, 선택 시 지정한 색(홈=골드/앰버, 번호 만들기=보라, 로또 연구소=다크 네이비, 내 번호=보라)으로 바뀌는지, (4) 홈 탭이 라이트/다크 테마 양쪽 모두 정상적인 대비로 보이는지.
- **롤백 안내**: 이번 라운드는 사용자가 이미 서로 다른 성격의 두 커밋으로 나눠 진행했다 — (1) 폰트/로또공 원복(`app/_layout.tsx`, `src/components/LottoBall.tsx`) — `d9b7383`으로 커밋·푸시 완료, (2) 탭 아이콘 PNG 원본 복원(`assets/tab-icons` 5장, `git checkout`) — `2d7069c`로 커밋·푸시 완료. 이번에 기기에 반영한 나머지 파일(`app/(tabs)/_layout.tsx` 재작성 + `lab.png` 네이비 재색칠 + 회색 PNG 5장 신규)은 아직 커밋되지 않은 세 번째 묶음이므로, 별도의 새 커밋으로 진행하길 권한다 — 문제가 생기면 이 커밋만 되돌리면 폰트/로또공 원복과 PNG 원본 복원은 그대로 둔 채 탭 아이콘 회색·컬러 전환 기능만 되돌릴 수 있다.

### 131. [실기기 QA 후속] 딥 패턴 아이콘 톤 통일 + 폰트 완전 원복(볼드 포함) + 다크모드 재점검 + 텍스트 무결성 점검
- **배경**: 130번 반영 후 사용자가 "번호 만들기" 탭 스크린샷 2장(동일 화면, "딥 패턴 탐색" 카드에 빨간 박스)과 함께 4가지를 요청했다 — ① "딥 패턴 탐색" 카드 아이콘만 다른 5개와 다르게 보이니 점·선에 색을 넣고 톤앤매너를 맞출 것, ② 해당 탭의 폰트가 정말 다 원복됐는지(볼드 여부 포함) 확인, ③ 다크모드 지원 전반 재점검, ④ 깨진 글자가 없는지 점검.
- **작업 1 — 딥 패턴 아이콘 톤앤매너 정리**: 다른 5개 메뉴 아이콘(`assets/quick-menu-icons/*.png`)을 실제로 열어 비교한 결과, 전부 "흰색 라운드 사각형 + 은은한 그림자 + 색감 있는 입체 그래픽" 조합이었다. `DeepPatternIcon.tsx`(SVG로 직접 그리는 유일한 아이콘)만 `accentViolet.primary` 단색 배경 위에 흰 점 + 연보라 선이라 무채색에 가깝게 보여 동떨어져 있었다. `react-native-svg`의 `LinearGradient`/`RadialGradient`로 배경을 다른 아이콘들과 같은 흰색 라운드 사각형(+ accentViolet 톤 그림자)으로 바꾸고, 선에는 `accentViolet.primary`→`.light` 그라디언트, 점(6개)에는 방사형 그라디언트를 입혀 옅은 입체감 있는 "보라 구슬 + 그라디언트 선" 형태로 다시 그렸다. LottoBall에 있었던 것과 같은 두꺼운 하이라이트/셰이딩 3D 효과(사용자가 직전에 명시적으로 원복 요청한 스타일)는 재사용하지 않고 그라디언트 수준으로 절제했다 — cairosvg로 동일한 SVG를 렌더링해 흰 배경 카드 위에서 실제로 다른 5개와 톤이 맞는지 시각 확인했다.
- **작업 2 — 폰트 완전 원복(볼드 포함)**: 130번에서는 `app/_layout.tsx`의 `useFonts` 로딩만 끄고 화면 곳곳의 `fontFamily: fontFamily.bold` 스타일 참조(18개 파일, 약 42곳 + 레이아웃 헤더 2곳)는 "RN이 시스템 폰트로 조용히 폴백해주니 안전하다"는 판단으로 남겨뒀었다. 그런데 사용자가 "볼드 여부도 원복해야 함"이라고 명시적으로 재차 지적했고, 점검해보니 Android에서는 미등록 `fontFamily` 문자열이 `fontWeight`와 함께 지정된 상태에서 폴백 타이프페이스를 고르는 과정에 굵기가 온전히 반영되지 않는 경우가 있다는 걸 확인했다 — 남겨뒀던 `fontFamily.bold` 참조 자체가 실기기에서 목격된 "볼드가 옅어 보이는" 원인일 가능성이 컸다. `fontFamily: fontFamily.bold` 스타일 프로퍼티를 화면·공용 컴포넌트 18개 파일에서 전부 제거하고(인라인/한 줄짜리 모두), 더 이상 쓰이지 않게 된 `fontFamily` import도 각 파일에서 같이 정리했다. `app/_layout.tsx`/`app/generate/_layout.tsx`의 `headerTitleStyle: { fontFamily: fontFamily.bold }`도 통째로 제거해(다른 프로퍼티가 없는 단일 속성 객체였다) React Navigation 기본 헤더 제목 스타일로 되돌렸다. 원래 있던 `fontWeight` 값은 전부 그대로 뒀다 — 130번 이전(Phase 5b/5c) 기록을 보면 그 fontFamily.bold는 항상 "이미 `fontWeight` 600 이상이던 제목/버튼급 텍스트"에만 얹는 방식이었어서, fontFamily만 걷어내면 원래 굵기로 정확히 돌아간다. `assets/fonts/*.otf`, `src/theme/typography.ts`의 `fontFamily` 토큰 자체는 나중에 재도입할 경우를 위해 남겨뒀다. 수정 후 문법(중괄호/괄호 짝), import 정리, 스타일 객체 구조를 전부 재확인했다.
- **작업 3 — 다크모드 재점검**: 앱 전체의 하드코딩 hex 컬러(120여 곳)를 훑어 라이트/다크 전환 시 문제가 될 만한 지점을 재확인했다. 대부분(버튼 위 흰 텍스트, 중립 그림자, "항상 어두운" 카드류 — `ProbabilityCard`/`destiny.tsx`의 진행률 화면/`ai-search.tsx`의 부스터 진행률 화면 등, 로또공 고유색 위 텍스트)은 과거 세션에서 이미 검토·확정된 의도적 고정값이라 문제가 없었다. 다만 `src/components/Dice45.tsx`(45면체 주사위 구슬)에서 실제 대비 계산으로 문제를 하나 발견했다 — 구슬 자체(`#0B1220`, 항상 어두운 장식)는 문제없지만, 그 아래 캡션("굴리는 중...", "방금 N 확정")이 라이트 테마의 `textMuted`(`#64748B`)로 고정돼 있어서, 다크 모드 화면 배경(`colors.background`, `#0B1220`) 위에서 대비비가 약 3.9:1로 WCAG AA(4.5:1) 기준에 못 미쳤다(라이트 배경에서는 문제없었다). `useAppTheme()`를 추가로 불러와 캡션 색을 `colors.textMuted`로 바꿔 다크 모드에서 `#94A3B8`(대비비 약 7.4:1)로 자동 전환되게 고쳤다. 이 외에 `PatternBoard.tsx`(로또 용지 시각화)는 이미 `colors.surface`/`border`/`textMuted`를 제대로 쓰고 있어 문제없음을 재확인했고, `preferences.tsx`/`BottomActionBar.tsx`/`dice.tsx`의 "비활성 버튼 연한 파랑"(`#BFDBFE`/`#93C5FD`)은 여러 컴포넌트에 걸쳐 일관되게 쓰이는 기존 관례(비활성 상태는 낮은 대비가 오히려 의도된 신호)라 이번엔 변경 대상에서 제외했다.
- **작업 4 — 텍스트 무결성 점검**: `app`/`src` 전체 소스에서 유니코드 치환문자(U+FFFD), 잘못된 UTF-8 바이트 시퀀스, 흔한 모지바케 패턴, JSX 안에 그대로 새는 `undefined`/`NaN` 리터럴을 전수 검색했다 — 전부 깨끗했다(파일 전체가 유효한 UTF-8이고, 화면에 잘못 노출되는 텍스트도 없음).
- **검증**: 수정한 20개 파일(아이콘 1개, Dice45 1개, 레이아웃 2개, 화면 14개, 공유 컴포넌트 2개) 전체를 다시 읽어 중괄호/괄호 짝, import 문 형태(이중 콤마·빈 import 없음)를 스크립트로 재확인했다. 이번에도 원격 브릿지 셸이 막혀(`sandbox-helper: no Plan9 drive shares mounted`) `npx tsc --noEmit`/`npx eslint .`는 실행하지 못했다 — 특히 이번 라운드는 18개 파일에서 동일한 패턴(`fontFamily: fontFamily.bold` 제거 + import 정리)을 기계적으로 반복 적용한 작업이라 단순 실수(빠뜨린 콤마 등) 가능성을 코드 검토로 최대한 좁혔지만, 사용자 로컬 `tsc`/`eslint` 확인이 이번에도 중요하다. 실기기 확인 포인트: (1) "딥 패턴 탐색" 카드 아이콘이 다른 5개와 비슷한 느낌(흰 배경+컬러 그래픽)으로 보이는지, (2) 번호 만들기 탭 전체(허브+10개 하위 화면) 제목·버튼 글자가 시스템 폰트로, 원래 굵기(진하게 보여야 할 곳은 여전히 진하게) 그대로 보이는지, (3) 시스템 다크모드로 전환 후 45면체 주사위 화면에서 "굴리는 중.../방금 N 확정" 캡션이 잘 읽히는지, (4) 다른 화면들도 다크모드에서 이상한 색이나 깨진 글자가 없는지 훑어보기.
- **롤백 안내**: 이번 라운드는 성격이 다른 네 가지 작업이 섞여 있지만 파일이 서로 겹치지 않아(아이콘 1개 / 폰트 관련 18개+레이아웃 2개 / 다크모드 Dice45 1개) 한 커밋으로 올려도 무방하다고 판단했다 — 다만 문제가 생기면 원인별로 되돌리기 쉽도록 아래 기준으로 나눠 올리는 것을 권장한다: (1) `src/components/deepPattern/DeepPatternIcon.tsx`(아이콘), (2) `app/_layout.tsx`/`app/generate/_layout.tsx` + 화면 14개 + `GeneratedGameCard.tsx`/`BottomActionBar.tsx`(폰트 원복), (3) `src/components/Dice45.tsx`(다크모드 캡션 대비).

### 132. [실기기 QA 후속] 로또 연구소 폰트 원복 확인 + "실제 당첨결과" 카드만 로또공 입체 효과 재도입
- **배경**: 131번 반영 후 사용자가 로또 연구소 탭 스크린샷(맨 위 "실제 당첨결과" 카드에 빨간 박스)과 함께 두 가지를 요청했다 — ① 이 탭도 폰트·볼드가 제대로 원복됐는지 확인, ② "실제 당첨결과" 카드(동행복권 공식 발표 데이터)의 공만 예외적으로 3D 효과를 살려서, 그 아래 통계 카드들(전부 그 결과를 가공한 값)과 "진짜 공식 데이터"라는 차이를 질감으로도 드러내고 싶다는 것 — "이런 섬세한 차이가 앱을 고급스럽게 한다"는 명확한 의도를 밝혔다.
- **확인 1 — 로또 연구소 폰트/볼드 원복**: `app/(tabs)/lab.tsx`의 `fontFamily`/`fontWeight` 전체를 재검사했다. `fontFamily` 참조는 (131번에서 이미) 전부 제거돼 있었고, `header`(800)/`cardTitle`(700)/`officialBadgeText`(700)/`weeklyTitle`(700)/`rowValue`(700)/`plusText`(700)/`transitionArrow`(600) 등 원래 있던 `fontWeight` 값은 전부 그대로 남아 있어 시스템 폰트 + 원래 굵기로 정상 원복된 상태임을 확인했다.
- **작업 — "실제 당첨결과" 카드만 로또공 입체 효과 재도입**: `LottoBall.tsx`를 다시 고치는 대신(전 화면에 영향을 주면 130번에서 원복한 취지와 어긋난다), `variant?: "flat" | "glossy"` prop을 새로 추가했다 — 기본값은 여전히 `"flat"`(130번에서 원복한 평면 단색+그림자)이라 이 prop을 넘기지 않는 다른 모든 화면(내 번호/선호번호/결과 카드/AI 조합 탐색 등 LottoBall을 쓰는 나머지 12곳)은 전혀 영향받지 않는다. `variant="glossy"`일 때만 원 안쪽에 클리핑 레이어를 추가로 두고, 이전에 지웠던 것과 같은 두 겹의 `LinearGradient` 오버레이(가장자리 셰이딩 `rgba(0,0,0,0.16)` + 좌상단 하이라이트 `rgba(255,255,255,0.55)`)를 되살렸다(그림자 자체는 바깥 View에 그대로 둬서 원 모양대로 클립되지 않게 했다). `app/(tabs)/lab.tsx`의 "실제 당첨결과" 카드 안 `LottoBall` 2곳(본번호 6개 + 보너스 번호)에만 `variant="glossy"`를 넘겼다. 반영 전 Python(PIL)으로 같은 좌표의 그라디언트를 별도 렌더링해 좌상단 하이라이트·우하단 셰이딩이 자연스러운 입체 구슬처럼 보이는지 시각 확인했다.
- **검증**: `LottoBall.tsx`/`app/(tabs)/lab.tsx` 문법(중괄호/괄호 짝) 확인, 저장소 전체에서 `<LottoBall` 사용처 17곳을 전수 재검색해 `lab.tsx`의 "실제 당첨결과" 카드 2곳만 `variant="glossy"`를 받고 나머지 15곳은 여전히 prop 없이(=기본값 `"flat"`) 호출되는지 확인했다. 이번에도 원격 브릿지 셸이 막혀 `npx tsc --noEmit`/`npx eslint .`는 실행하지 못했다 — `expo-linear-gradient`는 이미 다른 화면(`app/(tabs)/generate.tsx`의 하단 페이드 등)에서 쓰고 있던 의존성이라 새 패키지 설치는 필요 없다. 실기기 확인 포인트: (1) 로또 연구소 "실제 당첨결과" 카드의 공 7개(본번호 6 + 보너스)만 은은한 입체감(좌상단 밝고 우하단 어두운)이 보이는지, (2) 바로 아래 "번호별 출현 빈도"·"장기 미출현"·"이번 주 리포트" 등 다른 카드의 공은 여전히 평면인지(입체 효과가 실수로 새지 않았는지), (3) 다른 탭(내 번호·번호 만들기 결과 등)의 공도 전부 평면 그대로인지, (4) 로또 연구소 제목·카드 제목·배지 글자가 시스템 폰트 + 원래 굵기로 보이는지.
- **롤백 안내**: `src/components/LottoBall.tsx`(prop 추가, 하위 호환 — 기본 동작 불변) + `app/(tabs)/lab.tsx`(호출부 2곳) 두 파일이 한 세트라 한 커밋으로 올려도 되지만, 문제가 생기면 `lab.tsx`의 `variant="glossy"` 두 곳만 지워도 즉시 이전(전부 평면) 상태로 돌아간다 — `LottoBall.tsx` 자체를 되돌릴 필요는 없다.

### 133. [실기기 QA 후속] 내 번호 탭 — 저장된 번호도 로또공 입체 효과 + 폰트/볼드 재확인 + 다크모드 정밀 점검
- **배경**: 132번 반영 후 사용자가 내 번호 탭 스크린샷(저장된 번호 카드 3장, 각 공 나열에 빨간 박스)과 함께 요청했다 — "선택한 숫자에 해당하는 볼이므로" 여기 공에도 입체 효과를 살려달라는 것, 그리고 이 탭도 폰트·볼드 원복 여부와 다크모드 지원을 "디테일이 중요하다"며 필히 재확인해달라는 것.
- **작업 1 — 저장된 번호 공에 입체 효과 추가**: 132번에서 만든 `LottoBall`의 `variant="glossy"` 옵트인을 `app/(tabs)/tickets.tsx`의 저장된 번호 카드 한 곳(`item.game.numbers.map`으로 그리는 `ballRow`)에 적용했다. 로또 연구소의 "실제 당첨결과"와 같은 논리 — "가공되지 않은 실제 번호" — 라 `LottoBall.tsx` 상단 주석도 이제 두 사용처(실제 당첨결과 / 저장된 번호)를 함께 설명하도록 갱신했다. 이 화면에서 `LottoBall`은 이 한 곳에서만 쓰여서(저장소 전수 검색으로 확인) 다른 곳에 영향은 없다. 참고: 홈 화면("내가 자주 선택한 번호"), 결과 화면(아직 저장 전인 생성 직후 조합), QR 확인 화면(스캔 결과) 등 "숫자"가 나오는 다른 화면들은 이번 요청 범위(내 번호 탭)가 아니라 손대지 않았다 — 확대 적용은 별도 확인 후 진행하는 게 맞다고 판단했다.
- **확인 1 — 폰트/볼드 원복**: `app/(tabs)/tickets.tsx`의 `fontFamily`/`fontWeight` 전체를 재검사했다. `fontFamily` 참조는 없고, `emptyText`(700)/`sectionHeaderTitle`(800)/`sectionHeaderCountText`(700)/`rankText`(800)/`smallButtonText`(700)/`checkButtonText`(700)/`quickPickButtonText`(700)/`shareButtonText`(700)/`prefLinkText`(700) 등 원래 굵기가 전부 그대로 남아 있어 정상 원복 상태임을 확인했다.
- **작업 2 — 다크모드 정밀 점검에서 발견한 것**: 이 화면의 하드코딩 hex와 `colors`/`tints` 사용처를 전부 대조했다. "확인 완료" 등 상태 배지(`getStatusBadgeStyle`)는 이미 `tints.orange`/`tints.green`/`tints.indigo`/`tints.slate`를 써서 라이트/다크 모두 적절했고, 카드·구분선·헤더 배경도 전부 `colors.*` 토큰을 쓰고 있어 문제없었다. 다만 낙첨/등수를 보여주는 `rankText`(스크린샷의 "낙첨" 빨간 글자)가 `#DC2626` 고정값이었던 걸 실제로 계산해보니, 라이트 배경(흰 카드)에서는 대비비 약 4.8:1로 WCAG AA(4.5:1)를 겨우 만족했지만 다크 모드 카드 배경(`#161F32`)에서는 약 3.4:1로 기준 미달이었다. `statusBadgeText`가 이미 쓰고 있던 것과 같은 패턴(정적 스타일 + `tints` 인라인 색 오버라이드)으로 `tints.red.fg`를 쓰도록 고쳤다 — 라이트는 값이 동일해 변화 없고(`#DC2626` 그대로), 다크에서는 `#FCA5A5`(대비비 약 8.7:1)로 자동 전환된다.
- **검증**: `LottoBall.tsx`/`tickets.tsx` 문법(중괄호/괄호 짝) 확인, `<LottoBall` 사용처를 저장소 전수 재검색해 `variant="glossy"`가 정확히 3곳(로또 연구소 실제 당첨결과 2 + 내 번호 저장된 번호 1)에만 적용되고 나머지는 전부 기본값(`flat`)인지 재확인했다. `rankText` 대비비는 WCAG 공식(상대 휘도 기반)으로 직접 계산해 라이트/다크 각각 수치로 확인했다. 이번에도 원격 브릿지 셸이 막혀 `npx tsc --noEmit`/`npx eslint .`는 실행하지 못했다. 실기기 확인 포인트: (1) 내 번호 탭 저장된 카드들의 공이 로또 연구소 실제 당첨결과와 비슷한 입체감으로 보이는지, (2) 시스템 다크모드로 전환했을 때 "낙첨"/등수 텍스트가 카드 배경 위에서 잘 읽히는지(이전엔 흐릿하게 보였을 가능성), (3) 상태 배지("확인 완료" 등) 색이 다크에서도 배경과 구분되는지, (4) 이 탭의 제목·버튼 글자가 시스템 폰트 + 원래 굵기로 보이는지.
- **롤백 안내**: `app/(tabs)/tickets.tsx`(2곳 수정: glossy 적용, rankText 색상) + `src/components/LottoBall.tsx`(주석만 갱신, 동작 변경 없음) 두 파일. 문제가 생기면 `tickets.tsx`의 `variant="glossy"` 한 곳만 지우면 공 입체 효과만 되돌릴 수 있고, `rankText` 색상은 `{ color: tints.red.fg }`를 지우고 `styles.rankText`에 `color: "#DC2626"`을 다시 넣으면 이전 상태로 돌아간다(다만 그러면 다크모드 대비 문제가 재발한다).

### 134. [실기기 QA 후속] 45면체 주사위 — 검증 전용(공 입체 효과 없음 확인 + 폰트/볼드 + 다크모드 재확인, 수정 없음)
- **배경**: 133번 반영 후 사용자가 "45면체 주사위" 화면 스크린샷과 함께 — 이번엔 반대로 "여기는 볼에 3D 주면 안 됨"이라고 명시하며, 로또 연구소·내 번호와 달리 이 화면의 주사위 결과 공(6개)은 계속 평면(`flat`)이어야 한다고 확인했다. 같이 폰트·볼드 원복 여부와 다크모드 대응도 재확인해달라고 요청했다.
- **점검 결과 — 전부 정상, 코드 수정 없음**:
  - **공 입체 효과**: `app/generate/dice.tsx`의 `LottoBall` 호출(재굴리기 가능한 6개 슬롯, `<LottoBall number={n} size={48} />`)은 132/133번에서 `variant="glossy"`를 적용한 로또 연구소·내 번호와 달리 `variant`를 넘기지 않아 기본값 `"flat"` 그대로다 — 저장소 전수 검색으로 이 파일에 `variant="glossy"`가 전혀 없음을 재확인했다. 사용자가 원하는 상태와 정확히 일치해 손댈 게 없었다.
  - **폰트/볼드**: `app/generate/dice.tsx`와 화면 중심부인 `src/components/Dice45.tsx` 둘 다 `fontFamily` 참조가 전혀 없고(애초에 이 두 파일은 Phase 5b/5c의 "제목/버튼급 텍스트 Pretendard 적용" 대상에 포함된 적이 없다), `title`(700)/`buttonPrimaryText`(700)/`buttonOutlineText`(700)/`buttonSecondaryText`(700)/`emptySlotText`(700)/`toggleLink`(600) 등 원래 굵기가 전부 그대로였다.
  - **다크모드**: `dice.tsx` 자체의 스타일(컨테이너·버튼 3종·빈 슬롯·힌트·토글 링크)은 전부 `colors.*`/`brand.*` 토큰만 쓰고 있어 문제없었다. 이 화면이 함께 쓰는 공유 컴포넌트 `Dice45`(131번에서 캡션 대비 문제를 이미 고쳤다)·`NumberGrid`·`DisclaimerCard`·`BottomActionBar`도 전부 다시 열어 대조했고, 전부 `useAppTheme()`의 `colors`를 제대로 써서 라이트/다크 모두 적절했다(`NumberGrid`의 `cellTextSelected`처럼 "공 색 위 텍스트라 테마 무관 고정"인 값들은 기존에 이미 검토·문서화된 의도된 예외였다).
- **결론**: 이번 라운드는 확인 요청 3가지가 전부 "이미 맞는 상태"였다 — 별도 커밋 없이 QA_LOG 기록만 남긴다.

### 135. [홈 히어로 신규] "AI로 번호 만들기" CTA를 구체→캡슐 단일 오브젝트 모프 애니메이션으로 교체
- **배경**: 사용자가 레퍼런스 이미지 3장(옅은 샴페인색 구체가 진네이비 CTA 버튼으로 서서히 변형되는 3단계 목업)과 함께, GPT로 작성한 20개 항목짜리 상세 스펙을 전달하며 홈 탭 최상단 히어로 영역을 "더 과감하고 혁신적으로" 바꿔달라고 요청했다. 스펙의 핵심 요구는 (1) 구체와 완성된 CTA 버튼이 별도 오브젝트의 Crossfade가 아니라 **동일한 하나의 오브젝트**가 형태·색·그림자를 연속적으로 바꾸는 것, (2) width만 오른쪽으로 늘어나는 원→캡슐 변형(height 고정으로 radius=height/2 유지), (3) 샴페인→라벤더블루→미디움블루→딥네이비 5단계 색 보간, (4) 700~800ms·Bounce/Elastic/Overshoot 금지, (5) "AI로 번호 만들기" 텍스트(화살표 아이콘 없이)는 애니메이션 마지막 ~35%에서만 같은 오브젝트 위에 나타남, (6) 탭에 "진입"할 때마다(스크롤·상태 갱신이 아니라 포커스 기준) 재생, (7) 모프 중 탭해도 기존 클릭 핸들러가 즉시 동작, (8) 시스템 "동작 줄이기" 대응, (9) 기존 클릭 핸들러·네비게이션·문구는 그대로 유지.
- **작업**:
  - `src/components/HeroCtaMorph.tsx` 신규 작성 — `Animated.Value` 하나(`progress`, 0→1)를 `width`/`backgroundColor`/`shadowOpacity`/텍스트 `opacity`·`translateY`·왼쪽 고정 하이라이트 `opacity`에 전부 물려서 단일 `Animated.View` 위에서 보간한다(별도 구체 이미지 + 별도 버튼 + Crossfade 방식은 쓰지 않음). 왼쪽은 고정, 오른쪽으로만 늘어나도록 트랙 컨테이너에 `alignItems: "flex-start"`를 명시(기본값 stretch를 막는 것이 핵심). `Animated.timing`은 750ms, `Easing.bezier(0.22, 0.61, 0.36, 1)`(바운스·탄성·오버슈트 없는 절제된 ease-out)를 사용. 색상은 `#F6E7C9`(옅은 샴페인) → `#E7D6D6`(샴페인+라벤더) → `#B9C4E8`(옅은 라벤더블루) → `brand.primary`(미디움 블루) → `brand.dark`(딥 네이비) 5단계로, 마지막 두 단계는 임의색이 아니라 **앱이 이미 쓰는 브랜드 토큰**을 그대로 사용해 완성된 버튼이 다른 CTA들과 같은 톤이 되게 했다. CTA 텍스트는 진행률 0.62 지점부터만 나타나도록 opacity/translateY를 보간했다(화살표 아이콘 없음, 스펙대로). 히트 영역(`Pressable`)은 처음부터 트랙 전체 너비로 고정해둬서, 오브젝트가 아직 작은 원일 때 탭해도 750ms를 기다리지 않고 즉시 `onPress`가 동작한다. `AccessibilityInfo.isReduceMotionEnabled()`를 확인해 "동작 줄이기"가 켜져 있으면 애니메이션 없이 바로 완성된 CTA 상태로 보여준다(이 경우에도 CTA 기능 자체는 그대로 살아있음). 재생 트리거는 `useFocusEffect`로, 다른 탭에 갔다가 홈으로 돌아올 때마다(포커스를 다시 받을 때마다) 정확히 1회 재생되고 스크롤이나 상태 갱신으로는 재실행되지 않는다.
  - **최초 재생 타이밍 버그 발견 및 수정**: `useFocusEffect`(포커스 시 재생 트리거)와 `onLayout`(트랙의 실제 너비 확정)은 둘 다 비동기라 어느 쪽이 먼저 도착할지 보장되지 않는다. 특히 최초 마운트 시 focus가 layout보다 먼저 오면, 너비 보간의 목표값(`maxWidth`)이 아직 구체 크기(`ORB_SIZE`)로 남아있는 채로 애니메이션이 시작돼 "너비는 안 변하고 색만 바뀌는" 결과가 될 수 있었다. `trackWidthRef`/`pendingPlayRef`로 두 이벤트를 분리해, layout을 먼저 알기 전까지는 재생을 "예약"만 해두고 `onLayout`이 들어오는 즉시(또는 반대 순서면 focus 시점에 이미 알고 있는 너비로 즉시) 재생하도록 고쳐서, 도착 순서에 관계없이 최초 진입부터 항상 정상적으로 너비가 변하도록 했다.
  - `src/components/index.ts`에 `HeroCtaMorph` export 추가.
  - `app/(tabs)/index.tsx`의 기존 정적 `heroCard` + 그라디언트 CTA 버튼 JSX를 `<HeroCtaMorph subtitle title="이번 주 운명을 만들어보세요" ctaLabel="AI로 번호 만들기" onPress={...} accessibilityLabel={...} />` 한 줄로 교체 — 기존 로딩 스켈레톤 문구 로직, 클릭 핸들러(`router.push("/generate/ai-search")`), 접근성 라벨은 전부 그대로 유지했다. 더 이상 쓰지 않는 `LinearGradient` import와 `heroCard`/`heroSubtitle`/`heroTitle`/`ctaButton*` 스타일을 정리했다(로딩 스켈레톤에서 여전히 쓰는 `heroSkeletonSubtitle`/`skeletonTitle`은 남겨둠).
- **정직하게 밝혀야 할 절충점 2가지**:
  1. **배경 그래픽은 근사치다.** 레퍼런스 이미지의 정확한 리본/기하학적 배경 아트워크는 이미지 생성 도구 없이는 그대로 재현할 수 없어, `LinearGradient`(파스텔 블루~라벤더 톤) + 반투명 흰색 "블롭" View 2개로 대체했다. 색 분위기와 방향성은 맞췄지만 레퍼런스와 픽셀 단위로 같은 그림은 아니다.
  2. **새 네이티브 의존성을 추가하지 않았다.** 스펙에서 상정했을 법한 `react-native-reanimated`/`expo-blur` 등은 설치하지 않고, 이미 설치돼 있는 `expo-linear-gradient`와 RN 코어 `Animated` API만으로 구현했다 — 이번 세션 내내 `device_bash`(사용자 PC에서 명령 실행)가 `sandbox-helper: no Plan9 drive shares mounted` 오류로 막혀 있어 `npm install`/네이티브 재빌드를 대신 실행해드릴 수 없었기 때문이다. `width`/`backgroundColor`/`shadowOpacity` 보간이라 `useNativeDriver: false`로 JS 스레드에서 구동되는데, 750ms짜리 1회성 애니메이션이라 최신 기기에서는 체감상 문제없을 가능성이 높지만, 완전히 네이티브 드라이버로 도는 `reanimated` 기반 구현보다는 프레임 드랍 여지가 이론적으로 더 있다. 나중에 `npm install`이 가능한 환경이 되면 `reanimated`로 옮기는 걸 고려해볼 만하다.
- **검증**: 수정한 `HeroCtaMorph.tsx`의 중괄호/소괄호/대괄호 개수를 스크립트로 대조해 균형이 맞는지 확인(`{` 74/`}` 74, `(` 71/`)` 71, `[` 22/`]` 22). `app/(tabs)/index.tsx`도 동일하게 대조했고, 삭제한 스타일 이름(`heroCard` 등)에 대한 남은 코드 참조가 없는지 grep으로 재확인했다. 실기기 빌드(EAS) 전 단계라, (1) 탭 최초 진입 시 너비 변화가 실제로 보이는지, (2) 다른 탭에 갔다가 돌아올 때마다 재생되는지, (3) 모프 도중 탭해도 클릭이 바로 되는지, (4) 레퍼런스 대비 전체적인 느낌(배경 근사치 포함)은 다음 실기기 빌드에서 사용자가 직접 확인 필요.
- **롤백 안내**: 새 파일 `src/components/HeroCtaMorph.tsx` 삭제 + `src/components/index.ts`에서 `export * from "./HeroCtaMorph";` 한 줄 삭제 + `app/(tabs)/index.tsx`를 git으로 이전 커밋 상태로 되돌리면(또는 git diff 참고해 `heroCard`/`ctaButton*` JSX·스타일을 수동 복원하면) 이전 정적 히어로 카드로 완전히 되돌아간다.

### 136. [히어로 CTA 후속] 배경을 근사치 대신 사용자가 제공한 실제 레퍼런스 에셋으로 교체 + CTA 오브젝트 색상 보정
- **배경**: 135번에서 `LinearGradient`+블롭으로 배경을 근사했던 것에 대해, 사용자가 "이거랑 너무 느낌이 달라. 이 이미지 그대로 재현해줘야한다고"라며 레퍼런스 이미지를 다시 보내고, 이미지 생성 도구가 없다는 안내에 대해 GPT로 목업을 만들 때 나온 원본 에셋 파일들(배경 그래픽 원본, CTA 오브젝트 PNG 여러 장)을 직접 보내줬다. "동일하게 만들어서 구성해줘"라는 요청에 따라, 더 이상 근사치를 만들 필요 없이 실제 파일을 그대로 앱에 반영했다.
- **작업**:
  - 사용자가 보낸 파일 중 리본+다이아몬드 패턴의 세로형 배경 원본(941×1672, 텍스트·오브젝트 없이 순수 배경만 있는 깨끗한 이미지)을 JPEG로 변환(품질 92, 약 120KB)해 `assets/hero/hero-bg.jpg`로 앱에 새로 추가했다.
  - `src/components/HeroCtaMorph.tsx`의 배경 레이어를 `LinearGradient` + 블롭 2개(근사치) 대신 `<Image source={require("../../assets/hero/hero-bg.jpg")} resizeMode="cover" />` 한 줄로 교체 — 이제 근사치가 아니라 레퍼런스와 동일한 실제 그래픽이다. `resizeMode="cover"`라 기기 화면 너비에 따라 카드의 실제 가로세로 비율이 달라져도 중앙 기준으로 자동 크롭되어 빈 여백 없이 항상 꽉 채워진다.
  - 배경 이미지의 카드 상단 영역이 흰색에 가까운 밝은 톤이라, 그 위에 얹히는 제목/부제 텍스트의 대비를 위해 기존 스크림(`rgba(8,12,30,0.10)` → `0.12`로 살짝 진하게)에 더해 `subtitle`/`title` 텍스트 자체에도 `textShadow`(옅은 남색, blur 6)를 추가해 이중으로 가독성을 보강했다.
  - 사용자가 함께 보낸 CTA 오브젝트(구체→캡슐 모양) PNG 여러 장은 배경 제거 과정에서 생긴 것으로 보이는 가장자리 색 노이즈(투명 배경에 무지개색 얼룩)가 있어, 그대로 화면에 쓰면 오히려 지저분해 보인다고 판단해 이미지 자체는 쓰지 않았다. 대신 그 PNG에서 실제 사용된 색상 값을 Python(PIL)으로 스포이드처럼 좌표를 찍어 추출해서(하이라이트 `#FDFCFC`, 오렌지 영역 `#FEE599`, 전환부 라벤더-핑크 `#E2C4F8`, 옅은 라벤더 블루 `#B6D7FD`) `COLOR_STOPS_OUTPUT`의 앞 3단계 색상을 이 실측값에 맞게 보정했다(마지막 2단계는 기존대로 앱 브랜드 토큰 유지). 오브젝트 자체는 계속 135번의 `Animated.View` 기반 width/backgroundColor 보간 구조 그대로다 — 색상만 실제 에셋에 더 가깝게 맞춘 것.
- **왜 오브젝트 이미지를 그대로 쓰지 않았는지(중요)**: 받은 PNG들은 투명 배경 추출 품질이 낮아(가장자리에 노란·빨강·시안 등 무작위 색 픽셀이 얼룩덜룩 섞여 있음) 실제 화면의 다양한 배경 위에 올리면 그 얼룩이 그대로 도드라져 보인다. 정적 배경(카드 전체를 채우는 사각형 이미지)은 이 문제가 없어 그대로 썼지만, 오브젝트는 테두리가 노출되는 형태라 품질이 중요해 애니메이션 가능한 벡터/색상 기반 구현을 유지하는 쪽을 택했다. 더 깨끗하게 추출된(안티앨리어싱 없는 매끈한 알파 채널) PNG를 받을 수 있다면 재검토할 수 있다.
- **검증**: `HeroCtaMorph.tsx`의 중괄호/소괄호/대괄호 개수 스크립트 대조로 균형 확인(`{` 68/`}` 68, `(` 81/`)` 81, `[` 23/`]` 23). `LinearGradient`/`blobA`/`blobB` 코드 참조가 남아있지 않은지 grep으로 재확인(주석의 과거 설명 문구 1곳만 남음, 코드에는 없음). 실기기 빌드에서 (1) 배경이 레퍼런스와 실제로 동일하게 보이는지, (2) 카드 상단 텍스트가 밝은 배경 위에서도 잘 읽히는지, (3) CTA 오브젝트 색 보정이 자연스러운지 사용자 확인 필요.
- **롤백 안내**: `assets/hero/hero-bg.jpg` 삭제 + `HeroCtaMorph.tsx`에서 `<Image source={HERO_BG} .../>` 부분을 135번 커밋의 `LinearGradient`+블롭 코드로 되돌리고 `COLOR_STOPS_OUTPUT` 앞 3단계 색상도 135번 값(`#F6E7C9`/`#E7D6D6`/`#B9C4E8`)으로 되돌리면 이전 근사치 버전으로 복귀한다.

### 137. [히어로 CTA 재구현] 색상 근사치를 버리고 사용자가 제공한 실제 CTA 오브젝트 이미지로 와이프 리빌(wipe-reveal) 방식 재구현 + 둥근 단면 버그 수정
- **배경**: 136번에서 배경만 실제 에셋으로 교체하고 CTA 오브젝트(구체→캡슐)는 여전히 `Animated.View`의 `width`/`backgroundColor`를 보간하는 근사치였다. 사용자가 이번엔 실제 CTA 오브젝트 원본(스프라이트 시트, 그리고 구체에서 캡슐까지 색·형태 변화가 한 장에 다 담긴 연속 이미지)을 추가로 보내며 "레퍼런스에 가장 가깝게 하고 첨부한 이미지를 써서 지금 구현한 것처럼 뚝뚝 끊기지 않고 자연스럽게 구체에서 CTA 버튼으로 변경되게" 요청했다 — 근사치가 아니라 실제 픽셀을 쓰되, 기존 버전의 매끄러움은 그대로 유지해야 하는 요구.
- **에셋 정리(Python/scipy)**: 받은 원본 PNG(2172×724)는 투명 배경 추출 과정에서 생긴 가장자리 색 노이즈(빨강·노랑·시안 얼룩)가 있었다. 처음엔 알파 임계값 + `binary_erosion`/`binary_opening`/`binary_closing`(이진 형태학 연산)으로 접근했으나, 오렌지 구체 부분이 거의 다 깎여 나가 실패했다 — 원인은 이 에셋이 단순한 하드엣지 노이즈가 아니라 부드러운 글로우/블룸(연속적인 알파 그라데이션)을 가진 이미지라, 이진 형태학이 근본적으로 안 맞는 도구였기 때문. `scipy.ndimage.median_filter(size=5)`를 RGBA 4채널 모두에 독립 적용하는 방식으로 바꿔서 글로우 구조는 그대로 두고 점 노이즈만 제거하는 데 성공했다. 이후 `distance_transform_edt`로 알파가 거의 0인 영역에 가장 가까운 유효 픽셀의 RGB를 채워 넣어(추후 다크 헤일로 방지) `assets/hero/cta-pill.png`(750×296, 154KB)로 저장했다.
- **와이프 리빌(wipe-reveal) 구현**: `cta-pill.png` 한 장 자체가 왼쪽=작은 오렌지 구체, 오른쪽=파란 캡슐로 좌→우 방향으로 색·형태 변화가 이미 공간적으로 그려져 있는 이미지라는 점에 착안해, "이미지를 통째로 두고 progress에 따라 왼쪽부터 점점 더 넓게 드러내는" 방식으로 구현했다. `overflow:"hidden"` + 애니메이션되는 `width`를 가진 `revealBox`(`Animated.View`) 안에, 항상 최종 크기(`FINAL_WIDTH=230 × OBJECT_HEIGHT≈91`)로 고정된 `<Image resizeMode="stretch">`를 왼쪽 정렬로 넣어, 컨테이너 폭이 늘어날수록 이미지의 더 오른쪽 부분이 드러나게 했다. 이미지 자체는 한 번도 리사이즈/스트레치되지 않는다(width/height가 원본과 동일 비율로 고정) — 여전히 "하나의 연속된 오브젝트가 변형"이라는 원래 스펙 원칙 그대로이며, 색이 코드 계산값이 아니라 실제 에셋 픽셀이라는 점만 다르다. `progress` 보간·750ms·`Easing.bezier(0.22, 0.61, 0.36, 1)`·`useFocusEffect`·reduce-motion 처리·텍스트 opacity(0.62 지점부터)는 135/136번 그대로 유지했다. 최종 폭이 화면 너비와 무관한 고정값이 되면서 예전의 `onLayout`/`trackWidthRef`/`pendingPlayRef` 타이밍 대응 로직 자체가 필요 없어져 제거했고, 더 이상 쓰지 않는 `brand` 색상 토큰 import와 `COLOR_STOPS_INPUT`/`OUTPUT`도 정리했다.
- **발견 및 수정한 버그(둥근 단면이 사각으로 잘리는 문제)**: PIL로 진행률 0/25/50/75/100% 5단계를 렌더링해 실물 반영 전 미리 확인하는 과정에서, `revealBox`에 `borderRadius`가 없으면 중간 진행률(25/50/75%)에서 드러난 이미지의 오른쪽(아직 안 드러난 쪽) 잘린 단면이 사각형으로 뚝 끊겨 보이는 버그를 발견했다. 원본 이미지 자체의 둥근 알파 모양은 progress=1(전체 폭)일 때만 자연스럽게 나타나고, 중간 단계의 "잘린 단면"은 단순 사각 크롭이라 무조건 직각이었기 때문. `revealBox` 스타일에 `borderRadius: OBJECT_HEIGHT / 2`를 추가해, RN 네이티브 뷰 클리핑이 현재(애니메이션 중인) width 기준으로 매 프레임 둥글게 잘라내도록 고쳤다. 수정 전/후를 PIL 프리뷰로 나란히 비교해 모든 중간 단계에서 캡슐 끝처럼 둥글게 보이는 것을 확인 후 적용했다.
- **절충점(트레이드오프)**: 실제 에셋은 고정 가로세로 비율(약 2.53:1)을 가진 "완성된 그림"이라, 화면 폭 전체로 늘어나 채우도록 만들 수 없다(늘리면 구체가 타원으로 찌그러진다). 그래서 최종 버튼 폭을 화면 폭과 무관한 고정값(`FINAL_WIDTH=230`)으로 뒀다 — 레퍼런스 목업 자체도 버튼이 카드 전체 폭을 채우지 않고 좌우 여백을 두고 있어, 오히려 이 편이 레퍼런스에 더 가깝다고 판단했다. 136번에서 우려했던 "완성 상태 색이 브랜드 네이비(`#0F172A`)라 레퍼런스의 밝은 블루보다 너무 어둡다"는 지적은, 이번 버전에서 색이 코드 계산이 아니라 실제 에셋 픽셀(끝단이 선명한 블루)에서 그대로 나오게 되면서 자연히 해소됐다.
- **검증**: `HeroCtaMorph.tsx` 중괄호/소괄호/대괄호 균형 확인(`{` 56/`}` 56, `(` 71/`)` 71, `[` 12/`]` 12). grep으로 이전 버전의 잔재(`trackWidth`, `handleTrackLayout`, `COLOR_STOPS`, `morphObject`, `highlightOpacity`, `pendingPlayRef`) 코드에 남아있지 않은지 재확인(전부 없음). PIL 프리뷰 스크립트로 실제 컴포넌트의 보간 로직(revealWidth, borderRadius 클리핑 포함)을 그대로 재현해 5단계 진행률을 렌더링, 둥근 단면 수정 전/후 시각 비교 완료. 실기기 빌드에서 (1) 구체→CTA 버튼 전환이 뚝뚝 끊기지 않고 자연스러운지, (2) 모든 중간 단계에서 우측 단면이 둥글게 보이는지(사각으로 잘리지 않는지), (3) 레퍼런스 대비 전체적인 색·형태 충실도, (4) 텍스트 가독성/정렬, (5) 모프 도중 탭해도 즉시 동작하는지 사용자 확인 필요.
- **롤백 안내**: `src/components/HeroCtaMorph.tsx`를 136번 커밋(`e515cf3`) 상태로 되돌리고 `assets/hero/cta-pill.png`를 삭제하면 이전(배경만 실제 에셋, 오브젝트는 색상 보간) 버전으로 복귀한다.

### 138. [히어로 CTA 재구현 2차] 실제 이미지(137번)를 다시 걷어내고 Native UI(그라디언트/글로우/스페큘러 하이라이트)로 재구현
- **배경**: 137번에서 사용자가 보낸 실제 PNG(`cta-pill.png`)를 와이프 리빌 방식으로 그대로 화면에 반영했는데, 사용자가 바로 "짐 제작된 cta 이미지가 적합하지 않아 — 첨부한 이미지는 재질과 형태를 참고하는 레퍼런스야. 스프라이트 애니메이션으로 쓰지 말고 Native UI로 재현해!"라고 피드백을 줬다. 즉 실제 픽셀을 그대로 쓰는 게 아니라, 그 이미지가 보여주는 재질(유리질 광택·스페큘러 반점·바깥 글로우)과 형태(구체→캡슐)만 참고해서 코드로 다시 그려달라는 요청 — 137번의 접근 방향 자체가 틀렸던 것.
- **작업**: `src/components/HeroCtaMorph.tsx`를 다시 전면 재작성했다.
  - 오브젝트 렌더링 방식을 137번(이미지 크롭+와이프 리빌)에서 135번(코드로 그린 `Animated.View`)으로 되돌렸다 — `progress` 하나로 `width`(구체 지름 60→트랙 실측 전체 폭)와 `backgroundColor`를 함께 보간하는 단일 오브젝트 구조. `cta-pill.png` 참조와 `FINAL_WIDTH`/`ORB_REVEAL_WIDTH`/와이프 리빌 관련 코드는 전부 제거했다.
  - **색상은 135번(앱 브랜드 토큰, 진네이비로 끝남)으로 되돌리지 않고**, 사용자가 보낸 실제 레퍼런스 PNG(`3913525a-image.png`)에서 Python(PIL)으로 x좌표 15/25/40~55/70% 지점을 직접 스포이드해 추출한 값으로 새로 캘리브레이션했다: `#FFE7A6 → #FDBE55 → #DCB9F2 → #6E8FFC → #3D79FE`. 마지막 색이 선명한 블루(`#3D79FE`)라, 136번에서 지적했던 "완성 상태가 브랜드 네이비라 레퍼런스보다 너무 어둡다"는 우려가 이번에도 해소된다.
  - 레퍼런스의 "재질" 3요소를 순수 RN 뷰 + `expo-linear-gradient`(이미 설치돼 있음, 새 의존성 없음)만으로 재현: (1) 대각선 유리질 하이라이트 — `LinearGradient`(흰색 투명→반투명 32%→투명, 대각선 방향)를 오브젝트 안쪽에 `overflow:"hidden"`으로 클립. (2) 스페큘러 반점 — 왼쪽 위 고정 좌표에 반투명 흰 원 3겹(작고 진함+크고 옅음)을 겹쳐 부드러운 하이라이트처럼 보이게 함. (3) 바깥 글로우 — 오브젝트보다 한 단계씩 더 크고(패딩 12/26/44dp) 더 투명한(불투명도 0.22/0.16/0.10) 같은 색의 캡슐 레이어 3겹을 오브젝트 뒤에 겹쳐 후광을 흉내냄(CSS 다중 box-shadow와 같은 원리) — `Animated.add(width, pad)`로 각 글로우 레이어의 width도 본체와 함께 애니메이션되게 했다.
  - 오브젝트가 더 이상 고정 비율 이미지가 아니므로, 2차(137번)에서 버렸던 135번의 동적 전체 폭 로직(`onLayout`으로 트랙 실폭 측정 + `trackWidthRef`/`pendingPlayRef`로 포커스·레이아웃 이벤트 도착 순서 상관없이 최초 진입부터 정상 재생되게 하는 타이밍 처리)을 복원했다. 사용자가 보낸 3단계 목업 레퍼런스도 완성된 버튼이 카드 폭 거의 전체를 채우고 있어, 고정된 좁은 폭(137번의 230)보다 이쪽이 레퍼런스에 더 가깝다.
- **정직하게 밝혀야 할 한계**: RN에는 크로스플랫폼으로 동작하는 진짜 가우시안 블러가 없다(이 프로젝트엔 `expo-blur`/`reanimated`/Skia 미설치, 여전히 `device_bash`로 `npm install`을 대신 실행해드릴 수 없는 상태라 새 네이티브 의존성을 추가하지 않았다). 그래서 글로우는 "진짜 블러"가 아니라 크기·불투명도가 다른 레이어 3겹을 겹치는 근사치다 — 실기기 화면 크기에서는 충분히 은은해 보일 가능성이 높지만, 아주 가까이서 보면 레퍼런스만큼 완전히 매끈한 블러는 아닐 수 있다. 더 매끈하게 만들려면 `expo-blur` 설치 후 글로우 레이어에 `BlurView`를 씌우는 걸 다음 단계로 고려할 수 있다.
- **정리 필요(사용자 작업)**: 137번에서 추가했던 `assets/hero/cta-pill.png`는 이번 버전에서 더 이상 쓰지 않는다. 아직 git 커밋 전 단계였을 것으로 보이니(직전 안내드린 커밋 명령을 실행하기 전에 피드백을 주심), 커밋하기 전에 작업 트리에서 `assets/hero/cta-pill.png` 파일을 삭제하고 진행해달라고 안내드린다(아래 커밋 명령에 포함).
- **검증**: `HeroCtaMorph.tsx` 중괄호/소괄호/대괄호 균형 확인(`{` 86/`}` 86, `(` 97/`)` 97, `[` 21/`]` 21). grep으로 137번의 잔재(`CTA_PILL`, `FINAL_WIDTH`, `ORB_REVEAL_WIDTH`, `revealBox`, `revealWidth`, `pillImage`)가 코드(주석 제외)에 남아있지 않은지 재확인(전부 없음, 주석 속 히스토리 설명 2곳만 남음). PIL로 진행률 0/15/30/45/60/75/90/100% 8단계를 렌더링해 컴포넌트의 실제 보간 로직(width/backgroundColor 보간, 글로우 3겹, 대각선 하이라이트, 스페큘러 반점)을 그대로 재현한 뒤 색상·재질이 레퍼런스에 가까운지 시각 확인 완료. 실기기 빌드에서 (1) 구체→CTA 버튼 전환이 자연스러운지, (2) 대각선 광택·스페큘러 반점·바깥 글로우가 레퍼런스와 비슷한 느낌을 주는지(특히 글로우가 너무 각지거나 인위적으로 보이지 않는지), (3) 완성 상태 블루 색상이 레퍼런스와 가까운지, (4) 텍스트 가독성, (5) 모프 도중 탭해도 즉시 동작하는지 사용자 확인 필요.
- **롤백 안내**: `src/components/HeroCtaMorph.tsx`를 136번 커밋(`e515cf3`) 상태로 되돌리면 이전(배경만 실제 에셋, 오브젝트는 135번 색상 보간) 버전으로 복귀한다. 137번(이미지 기반 와이프 리빌)은 커밋되지 않았다면 그냥 무시하면 된다.

### 139. [히어로 CTA 실기기 QA 대응] 모프 속도/이징 개선 + 완성 버튼에 남는 스페큘러 얼룩 제거 + 앱 세션당 1회만 재생
- **배경**: 사용자가 138번 버전을 EAS 프리뷰 빌드로 실기기에 설치해 직접 녹화한 영상과 스크린샷으로 QA했다. 지적 사항 3가지: (1) 구체→CTA 버튼 전환 속도가 너무 빠르고 자연스럽지 않음, (2) 완성된 버튼 왼쪽에 동그란 얼룩(스페큘러 하이라이트 반점)이 그대로 남아있어 어색함(스크린샷에 빨간 박스로 직접 표시해서 보내줌), (3) 홈 탭에 재진입할 때마다(다른 탭 갔다가 돌아올 때마다) 매번 재생되는데, 앱을 새로 켰을 때 홈 탭에 딱 1번만 재생하고 그 이후에는 애니메이션 없이 완성된 버튼이 바로 보여야 함(계속 볼 이유가 없다는 이유). 배경 이미지 자체는 "저 정도면 괜찮다"고 확인해주셔서 손대지 않았다.
- **작업**: `src/components/HeroCtaMorph.tsx` 수정.
  - **(1) 속도/자연스러움**: `MORPH_DURATION_MS`를 750→900ms로 늘리고, easing을 `Easing.bezier(0.22, 0.61, 0.36, 1)`에서 더 부드럽게 감속하는 `Easing.bezier(0.16, 1, 0.3, 1)`(흔히 "ease-out-expo"로 불리는 커브)로 교체했다. 여전히 Bounce/Elastic/Overshoot는 쓰지 않는다(원래 스펙 원칙 유지).
  - **(2) 완성 버튼에 남는 얼룩**: 스페큘러 반점은 "구체였던 부분에 남은 광택"이라는 의도로 만들었는데, 오브젝트가 완전히 캡슐이 된 뒤(진행률 0.6 이후)에도 고정 위치·고정 불투명도로 계속 남아있다 보니 완성된 버튼 위에 붙은 별개의 원형 얼룩처럼 보였다 — 사용자가 스크린샷에 빨간 박스로 정확히 짚어준 부분. `progress`에 따라 0.35 지점부터 옅어지기 시작해서 0.6 지점에는 완전히 사라지도록 `specularFade` 보간(`Animated.multiply`로 각 반점 레이어의 불투명도에 곱해서 적용)을 추가했다 — 구체 형태를 벗어나는 시점과 맞물려 하이라이트도 자연스럽게 사라지는 느낌으로 만들었다. PIL 프리뷰로 진행률 60/75/90/100%에서 얼룩이 완전히 사라지는지 시각 확인 후 반영했다.
  - **(3) 앱 세션당 1회 재생**: 모듈 스코프 변수 `hasPlayedOnceThisSession`(React state/ref가 아니라 파일 최상단 변수)을 추가해 "이번 앱 세션에서 이미 재생했는지"를 추적한다. React state/ref로 만들면 컴포넌트가 탭 전환으로 언마운트될 때 같이 초기화되므로, 컴포넌트 생명주기와 무관하게 JS 모듈이 살아있는 한(=앱을 완전히 새로 켜기 전까지) 유지되는 모듈 스코프 변수를 일부러 썼다. `useFocusEffect`가 실행될 때 이미 재생한 적이 있으면 `progress.setValue(1)`로 애니메이션 없이 즉시 완성 상태를 보여주고, 처음이면 그대로 재생 후 플래그를 올린다. 모프 도중 탭해도 즉시 `onPress`가 동작하는 기존 동작(히트 영역이 처음부터 트랙 전체 폭으로 고정)은 변경 없이 그대로 유지된다.
- **검증**: `HeroCtaMorph.tsx` 중괄호/소괄호/대괄호 균형 확인(`{` 88/`}` 88, `(` 124/`)` 124, `[` 24/`]` 24). PIL로 진행률 0~100% 8단계를 다시 렌더링해 스페큘러 반점이 60% 이후 완전히 사라지는지(특히 75/90/100%에서 얼룩이 안 보이는지) 시각 확인 완료. `hasPlayedOnceThisSession` 로직은 코드 리뷰로 검증(React Native 컴포넌트 재생성 시 모듈 스코프 변수가 초기화되지 않는 것은 JS 모듈 시스템의 기본 동작이라 별도 네이티브 테스트 없이도 신뢰 가능). 실기기 빌드에서 (1) 전환 속도가 실제로 더 자연스럽게 느껴지는지, (2) 완성된 버튼에 얼룩이 더 이상 없는지, (3) 앱을 완전히 종료했다가 다시 켰을 때만 애니메이션이 재생되고, 다른 탭 갔다가 홈으로 돌아올 때는 애니메이션 없이 바로 완성된 버튼이 보이는지, (4) 모프 도중 탭해도 여전히 즉시 동작하는지 사용자 확인 필요.
- **롤백 안내**: `src/components/HeroCtaMorph.tsx`를 138번 커밋(`7e25e6a`) 상태로 되돌리면 이번 3가지 수정 전(750ms 이징, 얼룩 남는 스페큘러, 매 focus마다 재생) 버전으로 복귀한다.

### 140. [히어로 CTA 완성도 보강] 시작 시 "반짝임(shine sweep)" 추가 + 형태가 색을 이끄는 타이밍으로 조정
- **배경**: 139번(속도/얼룩/재생 1회 제한) 수정 후 사용자가 "좀 더 매끄럽게 맨 처음 살짝 반짝하는 구체임을 인지할 수 있어야 하고, 자연스럽게 색상과 모양이 CTA 버튼으로 변형되는 것을 사용자가 체감하고 디자인적인 완성도가 높구나를 느낄 수 있어야 해. 맨 마지막 부분에 CTA 버튼이 깔끔하게 딱 CTA 버튼으로 존재해야만 해"라고 추가 피드백을 줬다. 즉 (1) 시작 순간에 "이건 반짝이는 구체다"를 명확히 인지시키는 연출, (2) 색·형태 변화가 등속이 아니라 디자인적으로 의도된 순서감을 가질 것, (3) 최종 상태는 잡음 없이 깔끔한 CTA 버튼이어야 한다는 요구.
- **작업**: `src/components/HeroCtaMorph.tsx` 수정.
  - **시작 시 반짝임(shine sweep) 추가**: 기존 `progress`와 별개인 `shine` `Animated.Value`를 새로 두고, 재생을 `Animated.sequence`로 구성했다 — (1) `shine` 0→1을 `SHINE_DURATION_MS`(220ms) 동안 먼저 재생한 뒤 (2) 기존 `progress` 0→1(900ms) 모프를 이어서 재생. `shine` 구간에는 오브젝트가 아직 정지된 구체 상태 그대로고, 대각선으로 기울어진 흰색 바 하나가 `pill`의 `overflow:"hidden"`(이 시점엔 원 모양으로 클립됨) 안에서 왼쪽 바깥→오른쪽 바깥으로 한 번 스치듯 지나가며 나타났다 사라진다(`shineOpacity`가 0→0.85→0으로 보간, `shine`이 1에 도달하면 완전히 투명해져 이후 모프·완성 상태에는 전혀 남지 않는다) — "빛이 유리구슬 표면에 잠깐 반사되는" 느낌을 줘서 재생 시작 순간 사용자가 "이건 반짝이는 구체구나"를 명확히 인지하게 했다.
  - **형태가 색을 이끄는 타이밍**: `COLOR_STOPS_INPUT`을 균등 분포(`[0, 0.25, 0.5, 0.75, 1]`)에서 뒤로 몰린 분포(`[0, 0.35, 0.6, 0.85, 1]`)로 바꿨다. `width`(형태)는 그대로 처음부터 매끄럽게 자라지만, 색은 초반엔 오렌지 톤에 더 오래 머물다가 진행률 후반에 블루로 집중적으로 바뀐다 — 모든 속성이 동시에 등속으로 바뀌는 것보다 "형태가 먼저 자리를 잡고 색이 뒤따라와 완성되는" 순서감을 줘서 더 디자인적으로 느껴지게 했다.
  - **깔끔한 최종 상태**: 138/139번에서 이미 스페큘러 반점을 진행률 0.6 이후 완전히 페이드아웃시켜뒀고(139번), 이번에 추가한 shine 바도 자기 자신의 재생이 끝나면(`shine`=1) opacity 0으로 완전히 사라지도록 설계해, 최종 완성 상태에는 캡슐 몸체 + 은은한 바깥 글로우 + 옅은 대각선 유리질 하이라이트만 남고 별도의 원형·바 형태 잔여물이 전혀 남지 않는다.
- **검증**: `HeroCtaMorph.tsx` 중괄호/소괄호/대괄호 균형 확인(`{` 97/`}` 97, `(` 152/`)` 152, `[` 34/`]` 34). PIL 프리뷰를 확장해 (a) shine 10/35/60/85% 스냅샷을 별도 렌더링해서 반짝임 바가 원 범위 안에서만 보이고 자연스럽게 스치는지, (b) progress 45%(아직 따뜻한 오렌지 톤 유지)와 75%(블루로 많이 진행) 프레임을 비교해 형태-색 순서감이 의도대로 체감되는지 시각 확인 완료. 실기기 빌드에서 (1) 재생 시작 순간 반짝임이 자연스럽게 보이는지(너무 튀거나 번쩍거리지 않는지), (2) 전체적으로 디자인 완성도가 더 높게 느껴지는지, (3) 완성된 버튼에 여전히 잡음 없이 깔끔한지, (4) 앱을 완전히 재시작했을 때만 재생되고 탭 전환 시에는 재생되지 않는지(139번 동작 유지 확인), (5) 모프 도중 탭해도 즉시 동작하는지 사용자 확인 필요.
- **롤백 안내**: `src/components/HeroCtaMorph.tsx`를 139번 커밋(`3270782`) 상태로 되돌리면 이번 반짝임/색 타이밍 조정 전 버전으로 복귀한다.

### 141. [히어로 CTA 전문가 톤 완성도 보강] 반짝임 절제 + 완성 순간 펄스 + 눌림 피드백 + 드롭섀도 추가
- **배경**: 140번(시작 반짝임 + 형태-색 타이밍 조정) 이후 사용자가 "최초 빛이 너무 쎄면 안됨. 자연스럽고 고급스럽게 빛나야 해. 그리고 CTA 버튼이 매끄럽게 완성되어야 해. 디자인의 완성도를 좀 더 끌어올려봐. 엄청 능숙한 전문 디자이너가 하는 방식으로 구현해줘. 그리고 버튼이 완성되면 유저가 버튼을 누르고 싶은 느낌이 나도록 되는지 한번 더 체크해봐"라고 요청했다. 정성적인 요청이라, 실제 전문 UI 모션/버튼 디자인에서 쓰이는 구체적인 기법들로 번역해 적용했다.
- **원인 진단(누르고 싶은 느낌 체크 결과)**: 코드를 다시 살펴보니 기존 `Pressable`에는 탭 시 시각적 피드백이 전혀 없었다(기능은 동작하지만 눌러도 아무 반응이 안 보임) — 이게 "누르고 싶은 느낌"이 부족했던 실제 원인 중 하나로 보인다. 또한 PIL 프리뷰 스크립트에는 처음부터 오브젝트 아래 은은한 드롭섀도가 들어가 있었는데, 실제 `HeroCtaMorph.tsx`에는 이 레이어를 옮겨 담는 걸 빠뜨렸다는 것도 이번에 발견했다 — 그래서 그동안 프리뷰보다 실물이 더 납작하고 배경에 눌려 보였을 가능성이 있다.
- **작업**: `src/components/HeroCtaMorph.tsx` 수정.
  - **반짝임 절제**: 단색 사각형(불투명도 최대 0.85, 플래토 구간 존재) 대신 가장자리가 부드럽게 페이드되는 `LinearGradient` 바(투명→아이보리 반투명→투명)로 교체하고, 폭도 줄이고(0.3→0.22×CTA_HEIGHT), 불투명도 곡선을 플래토 없는 삼각형 곡선(정점 0.5, 5차 버전의 0.85+긴 유지 구간에서 대폭 하향)으로 바꿨다. 순백 대신 아이보리 톤(`#FFF8E6`)을 써서 더 은은하고 고급스러운 느낌을 준다.
  - **완성 순간의 은은한 펄스**: `settle`이라는 세 번째 `Animated.Value`를 추가해, 모프가 끝난 직후 짧게(120ms 상승 + 260ms 하강) 0→1→0으로 움직이며 글로우 3겹의 불투명도를 순간 최대 1.5배까지 끌어올렸다가 되돌린다(`Animated.multiply`) — 오브젝트가 "완성되는 순간 살짝 빛나며 자리를 잡는" 느낌을 준다. Bounce/Elastic/Overshoot(위치·크기 튐)는 여전히 쓰지 않고 불투명도 펄스만 사용해 스펙 원칙을 지켰다.
  - **눌림(press) 피드백 추가**: `pressScale` `Animated.Value`를 추가해 `onPressIn`에 0.965로 빠르게(90ms), `onPressOut`에 1로 부드럽게(150ms) 복원되도록 했다 — 히트 영역 전체에 적용해 버튼을 누르는 순간 살짝 눌리는 촉각적 피드백을 준다.
  - **입체감(드롭섀도) 추가**: `shadowLayer`를 새로 추가했다(글로우 뒤, pill 앞) — iOS는 `shadowColor`/`shadowOffset`/`shadowOpacity`/`shadowRadius`, Android는 `elevation`(그림자가 실제로 그려지려면 불투명 배경이 필요해 pill과 같은 `animatedColor`를 그대로 깔아뒀다 — pill이 완전히 덮으므로 보이는 건 그림자 부분뿐). 살짝 아래로 떨어지는 남색 톤 그림자(`#13224D`)로 "카드 위에 얹힌 버튼" 같은 입체감을 준다.
- **검증**: `HeroCtaMorph.tsx` 중괄호/소괄호/대괄호 균형 확인(`{` 124/`}` 124, `(` 206/`)` 206, `[` 45/`]` 45). PIL 프리뷰를 확장해 (a) 새 반짝임 곡선을 shine 15/30/45/60/85%로 다시 렌더링해 이전보다 확실히 부드럽고 절제됐는지, (b) progress 100%에서 settle=0(평상시)과 settle=1(펄스 순간) 프레임을 나란히 렌더링해 과하지 않은지 시각 확인 완료. `pressScale`/드롭섀도는 순수 인터랙션·플랫폼 렌더링 요소라 정적 이미지로는 한계가 있어 코드 리뷰로 로직만 검증(Android `elevation`의 실제 렌더링 결과는 PIL로 재현 불가 — 실기기에서 최종 확인 필요). 실기기 빌드에서 (1) 반짝임이 과하지 않고 은은하게 느껴지는지, (2) 완성되는 순간 살짝 빛나며 자리 잡는 느낌이 나는지, (3) 버튼을 누를 때 살짝 눌리는 피드백이 보이는지, (4) 완성된 버튼이 카드 위에 떠 있는 듯한 입체감이 있는지(특히 Android에서 elevation 그림자가 의도대로 보이는지), (5) 전반적으로 "누르고 싶은" 느낌이 드는지 사용자 확인 필요.
- **롤백 안내**: `src/components/HeroCtaMorph.tsx`를 140번 커밋(`6e9bbe7`) 상태로 되돌리면 이번 4가지 보강(반짝임 절제/완성 펄스/눌림 피드백/드롭섀도) 전 버전으로 복귀한다.

### 142. [히어로 CTA "Apple 시니어 디자이너" 수준 완성도 보강] timing→spring 모션 교체 + 눌림 시 그림자 동반 감쇠 + 성능/용량 영향 없음 확인
- **배경**: 141번(반짝임 절제/완성 펄스/눌림 피드백/드롭섀도) 이후 사용자가 "좀 더 고급스럽고 마치 APPLE 시니어 디자이너가 만든 것 같은 느낌으로 완성도를 좀 더 높여봐. 더 높은 완성도가 가능할 거 같은데, 덜 만든 느낌이야. 그리고 이것때메 앱이 느려지거나 용량이 비약적으로 늘어선 안되는거 필히 체크해"라고 요청했다. 정성적 완성도 요구 + 명시적 성능/용량 검증 요구, 두 갈래로 대응했다.
- **작업 1 — timing에서 spring(물리 기반)으로 교체**: iOS 시스템 애니메이션 대부분이 정해진 시간에 곡선을 그리는 timing이 아니라 질량·강성·감쇠로 물리 시뮬레이션되는 spring 기반이라는 점에 착안해, 메인 모프(`progress`)를 `Animated.timing` + 커스텀 bezier에서 `Animated.spring({ stiffness: 40, damping: 14, mass: 1 })`으로 교체했다. `damping`을 이 stiffness/mass 조합의 임계감쇠(약 12.6)보다 살짝 높게(약 1.08배) 잡아 이론상 오버슈트가 없도록 했고, 여기에 `overshootClamping: true`까지 추가로 걸어서 물리 계산 오차와 무관하게 애니메이션 값이 목표치(1)를 절대 넘지 않도록 이중으로 보장했다 — "Bounce/Elastic/Overshoot 금지"라는 사용자의 원래 스펙 원칙은 그대로 지키면서, 정해진 곡선을 억지로 따라가는 느낌 대신 무게감이 느껴지는 유기적인 감속감만 얻었다. stiffness/damping 값은 기존 900ms 체감 속도와 비슷하게 맞춰서, 지금까지 조정해온 반짝임→모프→완성 펄스의 전체 타이밍 감각이 깨지지 않도록 했다. 더 이상 쓰지 않는 `MORPH_DURATION_MS` 상수와 그 커스텀 bezier easing은 제거했다.
- **작업 2 — 눌림 피드백 보강**: 누를 때 크기(scale)만 줄어드는 게 아니라 그림자/글로우도 함께 옅어지도록(`pressDim` — 기존 `pressScale`과 같은 값을 재사용, 별도 `Animated.Value` 추가 없음) 해서, 버튼이 표면 속으로 살짝 가라앉는 듯한 물리적 눌림감을 더했다 — iOS 네이티브 버튼의 눌림 반응(하이라이트가 옅어지며 눌리는 느낌)에 더 가깝다.
- **작업 3 — 성능/용량 영향 명시적 확인(사용자가 직접 요청)**: (1) 이번 히어로 CTA 기능 전체에 걸쳐 새 npm 패키지를 하나도 추가하지 않았다 — 처음부터 끝까지 이미 설치돼 있던 `expo-linear-gradient` 하나만 사용. `package.json`에 변경 없음(이번 세션에서 수정한 적 없음, `HeroCtaMorph.tsx`만 계속 편집). (2) 이미지 에셋은 136번에서 추가한 `assets/hero/hero-bg.jpg`(약 122KB) 하나뿐 — 실제로 `device_list_dir`로 `assets/hero/` 폴더를 다시 확인해, 137번에서 만들었다가 138번에서 이미지 방식을 포기하며 커밋 전에 삭제했던 `cta-pill.png`가 남아있지 않은 것도 재확인했다(파일 1개만 존재). (3) 애니메이션은 앱 세션당 딱 1번(반짝임 220ms + 모프(스프링, 기존 체감 속도와 비슷하게 튜닝) + 완성 펄스 약 380ms, 총 1.5초 내외)만 재생되고 끝나면 완전히 정지한다 — 반복 루프나 상시 대기 타이머가 전혀 없어 화면을 보고만 있을 때 추가 CPU/배터리 소모가 없다(139번에서 이미 세션당 1회 제한을 적용해둔 상태). (4) 각 레이어(글로우 3겹, 그림자 1겹, 본체, 스페큘러 3겹, 하이라이트/반짝임 그라디언트 각 1겹)는 전부 가벼운 `View`/`LinearGradient`이고 이미지 디코딩·네트워크 요청이 전혀 없다 — 즉 이 기능이 앱 번들 용량에 더하는 건 순수 JS/TSX 코드 몇 KB뿐이다.
- **검증**: `HeroCtaMorph.tsx` 중괄호/소괄호/대괄호 균형 확인(`{` 125/`}` 125, `(` 227/`)` 227, `[` 48/`]` 48). grep으로 더 이상 쓰지 않는 `MORPH_DURATION_MS`/구 bezier easing이 코드에 남아있지 않은지 확인(과거 히스토리 설명 주석에만 남아있고 실제 코드에는 없음). `device_list_dir`로 `assets/hero/` 폴더를 재확인해 이미지 에셋이 `hero-bg.jpg` 1개뿐임을 검증. spring 물리 파라미터(damping vs 임계감쇠)는 직접 계산으로 오버슈트 없음을 확인했고, `overshootClamping: true`를 추가로 걸어 이중 안전장치를 뒀다 — 다만 spring의 정확한 "체감 지속 시간"은 timing처럼 고정값이 아니라 물리 계산 결과라, 실기기에서 이전(900ms 근처) 대비 너무 빠르거나 느리게 느껴지지 않는지는 실물 확인이 필요하다. 실기기 빌드에서 (1) 모션이 전반적으로 더 "고급스럽고 자연스럽게" 느껴지는지, (2) 여전히 오버슈트/바운스 없이 절제돼 있는지, (3) 누를 때 그림자/글로우가 함께 옅어지는 게 자연스러운지, (4) 체감 속도가 이전과 비슷한지(너무 빠르거나 느려지지 않았는지), (5) 프레임 드랍이나 눈에 띄는 성능 저하가 없는지 사용자 확인 필요.
- **롤백 안내**: `src/components/HeroCtaMorph.tsx`를 141번 커밋(`24cdc51`) 상태로 되돌리면 이번 spring 교체 + 눌림 시 그림자 동반 감쇠 전 버전으로 복귀한다.

### 143. [히어로 CTA 정밀 재점검] spring 설정값 실측 버그(정착 2~2.5초) 수정 + 코어 셰이딩으로 구체 입체감 보강
- **배경**: 사용자가 "처음엔 구체 느낌이 명확해야 하고, 자연스럽고 고급스럽게 CTA 버튼으로 전환되는 걸 원한다. 전문 디자이너가 직접 심혈을 기울여서 만든 것처럼 되었는지 다시 정확하게(정밀하게) 체크해서 반영해달라"고 요청했다. 6~9차(139~142번)에 걸쳐 스크린샷/영상 없이 "Apple 시니어 디자이너 느낌", "덜 만든 느낌" 같은 정성적 피드백이 이어져 왔는데, 이번에는 특히 "정확하게 체크"를 명시적으로 요구해서, 눈대중으로 한 번 더 코스메틱하게 조정하는 대신 실제로 수치를 검증하는 방식으로 접근했다. 그 결과 142번에서 넣은 spring 설정값 자체가 의도와 전혀 다르게 동작하고 있던 회귀 버그를 발견했다.
- **작업 1 — [버그 수정] spring 정착 시간 실측 및 재조정**: 142번에서 `Animated.spring({ stiffness: 40, damping: 14, mass: 1 })`을 "기존 900ms 체감과 비슷할 것"이라는 어림짐작으로 넣었는데, 이번에 RN의 spring 적분 방식(semi-implicit Euler, `accel = (-stiffness*(x-1) - damping*v) / mass`, 60fps 스텝)을 Python으로 그대로 재현해 시뮬레이션해보니 실제 정착 시간이 약 2.0~2.5초로, 의도했던 900ms의 2배 이상이었다 — 지난 몇 차례 "덜 만든 느낌"/"매끄럽지 않다"는 피드백의 상당 부분이 실제로는 이 버그 때문이었을 가능성이 높다. 같은 감쇠비(damping ≈ 2.05×√stiffness, 임계감쇠보다 약 1.025배 — 오버슈트 없음 유지)를 유지하며 stiffness를 120~280 구간에서 다시 스윕해 정착 시간을 계산했다: stiffness 120→약 1.30초, 160→약 1.17초, 200→약 1.07초, 240→약 0.98초, 280→약 0.93초. 그중 `stiffness: 240, damping: 31.8`이 원래 의도했던 900ms~1초 체감에 가장 근접해 이 값으로 교체했다(`mass: 1`, `overshootClamping: true`는 그대로 유지). 교체 후 같은 시뮬레이터로 재검증한 결과 0.9초 시점에 이미 목표치의 99.98%(x≈0.9998)에 도달하고 약 0.98초에 완전히 정착하며, 어느 시점에도 목표치(1.0)를 넘지 않아 오버슈트 없음도 재확인했다.
- **작업 2 — [보강] 코어 셰이딩(core shading) 추가로 "구체 느낌" 강화**: 지금까지 오브젝트에는 왼쪽 위 대각선 하이라이트(밝음)만 있고 반대쪽에는 아무 셰이딩이 없어서, 확대해서 보면 "빛나는 평면 색 원반"처럼 읽힐 여지가 있었다. 실제 유리구슬/보석은 하이라이트 반대편에 옅은 그림자(ambient occlusion)가 있어야 표면이 굽어있다는 게 눈에 명확히 읽힌다 — 이번 피드백의 "구체 느낌이 명확해야" 요구에 직접 대응하는 보강이다. 오른쪽 아래로 갈수록 짙어지는 아주 옅은 남색 `LinearGradient`(`rgba(10,16,36,0)` → `rgba(10,16,36,0.2)`, locations `[0, 0.55, 1]`로 앞쪽 절반은 거의 투명하게 두고 뒤쪽 절반부터만 옅게 번지도록)를 기존 하이라이트 레이어 바로 뒤에 추가했다. PIL 프리뷰(`render_native.py`에 numpy 기반 셰이딩 프로토타입을 먼저 만들어 검증한 뒤 RN `LinearGradient`로 이식)로 progress 0%(구체 상태)와 100%(완성된 캡슐) 양쪽을 모두 확대해서 확인한 결과, 구체 상태에서는 입체감이 뚜렷하게 살아나고, 완성된 캡슐 상태에서도 과하게 어두워지지 않고 은은한 깊이감만 더해주는 정도로 자연스러웠다.
- **검증**: `HeroCtaMorph.tsx` 중괄호/소괄호/대괄호 균형 확인(`{` 133/`}` 133, `(` 253/`)` 253, `[` 54/`]` 54). grep으로 실제 `Animated.spring()` 호출부에는 새 값(`stiffness: 240`, `damping: 31.8`)만 남아있고, 옛 값(`stiffness: 40`, `damping: 14`)은 이번 회귀를 설명하는 히스토리 주석에만 남아있음을 확인. Python 물리 시뮬레이터로 새 spring 설정값이 약 0.98초에 정착하고 어느 시점에도 목표치를 넘지 않음(오버슈트 없음)을 재검증. `LinearGradient` 레이어 개수가 6개→8개로 늘어난 것(코어 셰이딩 1겹 추가)을 grep으로 확인. 새 npm 패키지·새 이미지 에셋은 이번에도 추가하지 않았다(순수 `LinearGradient` 레이어 1겹 + 기존 spring 파라미터 수정뿐) — 7차(141번)에서 확인한 성능/번들 용량 무영향 원칙 계속 유지됨. 다만 spring 체감 속도와 코어 셰이딩의 실제 화면상 느낌은 PIL 프리뷰로만 확인했고 실기기 확인은 아직 못 했다 — 실기기 빌드에서 (1) 모프가 이제 확실히 900ms~1초 근처로 느껴지는지(이전보다 훨씬 빨라짐), (2) 구체 상태에서 입체감이 이전보다 명확하게 느껴지는지, (3) 완성된 버튼에 코어 셰이딩이 과하거나 어색하게 보이지 않는지 사용자 확인 필요.
- **롤백 안내**: `src/components/HeroCtaMorph.tsx`를 142번 커밋(`9377841`) 상태로 되돌리면 이번 spring 값 수정 + 코어 셰이딩 추가 전 버전(버그가 있는 spring 값 그대로)으로 복귀한다.

### 144. [히어로 CTA 검증 전용] 사용자가 지적한 "왼쪽 캡 둥근 얼룩"은 실제 코드가 아니라 PIL 프리뷰 스크립트 자체의 렌더링 버그로 확인 — 코드 수정 없음
- **배경**: 143번 변경사항을 커밋·푸시한 직후, 사용자가 완성된 CTA 버튼(progress 100%) 스크린샷에 빨간 박스로 왼쪽 캡 부분을 표시해 보내왔다(별도 설명 텍스트 없음). 확인해보니 이 스크린샷은 실제 기기 캡처가 아니라 이번 세션에서 내가 직접 만들어 검토했던 `render_native.py`의 `crop_progress100.png` 미리보기 이미지였다("progress 100%" 라벨이 그대로 찍혀 있어 확인 가능) — 즉 사용자가 내 검증용 프리뷰 결과물 자체에서 문제를 발견하고 되돌려준 것이다.
- **조사**: 빨간 박스 영역을 좌표 검출(`red_mask`) 후 3배 확대해서 보니, 하이라이트가 매끄러운 그라디언트가 아니라 왼쪽 캡 부분에만 뚜렷한 곡선 경계로 잘려 있어 "별도의 둥근 조각이 붙은" 것처럼 보였다. `render_native.py`의 대각선 하이라이트 코드를 확인한 결과, 실제 `expo-linear-gradient`처럼 오브젝트 폭에 비례하는 분수 좌표가 아니라 `band_w = obj.size[1] * 1.3`(즉 **높이에만 비례하는 고정 픽셀 크기**) 폴리곤으로 그려지고 있었다 — 오브젝트가 정사각형(구체)일 때는 우연히 맞아떨어지지만, progress=100%로 폭이 크게 늘어나면 이 고정 크기 밴드가 왼쪽 캡 부근에만 남아 폭 전체에 걸쳐 스케일되지 않는다. Python으로 실제 `expo-linear-gradient`의 좌표 투영 수식(`start`/`end`가 컴포넌트 자신의 width/height 대비 분수, 그 사이를 잇는 벡터에 각 픽셀을 투영해 t를 구하고 `locations`로 보간)을 그대로 재현해서 비교 계산한 결과, 실제 TSX 컴포넌트가 쓰는 진짜 `LinearGradient`는 폭이 넓어질수록 그라디언트 방향이 거의 수평에 가깝게 자동으로 눌리며 매끄럽게 스케일되는 것으로 확인됐다(예: width=300/height=60에서 x=0 지점 alpha≈0.03, x=33%에서 피크 alpha≈0.32, x=65% 이후 alpha≈0) — 즉 실제 컴포넌트는 이 문제의 영향을 받지 않는다.
- **결론 — 코드 수정 없음**: `src/components/HeroCtaMorph.tsx`는 처음부터 `expo-linear-gradient`의 분수 좌표(`start`/`end` %)를 올바르게 쓰고 있어 수정할 필요가 없다. 대신 검증 도구인 `render_native.py`(스크래치패드, 저장소에 포함되지 않음)의 하이라이트 밴드와 143번에서 추가한 코어 셰이딩 그라디언트를 모두 실제 `LinearGradient`와 동일한 수학(신규 헬퍼 `linear_gradient_layer` — start/end 분수 좌표를 픽셀로 변환해 각 픽셀을 투영, `locations`로 알파 보간)으로 다시 구현해 프리뷰 정확도를 개선했다. 재렌더링한 progress 0%(구체 — 여전히 명확하게 둥글고 입체감 있음)/45%/100%(완성된 캡슐 — 하이라이트가 매끄럽게 번지고 갑자기 잘리는 경계 없음) 프레임을 모두 확대 확인한 결과 더 이상 둥근 얼룩이 보이지 않았다.
- **검증**: 빨간 박스 좌표를 numpy로 자동 검출해 정확한 문제 영역을 특정한 뒤 확대 검토, 실제 `expo-linear-gradient` 좌표 투영 수식을 Python으로 재현해 오브젝트 폭에 따른 그라디언트 형태 변화를 수치로 비교, 프리뷰 스크립트를 동일 수식으로 재작성 후 재렌더링한 3개 진행률(0/45/100%) 프레임을 육안 재확인. `HeroCtaMorph.tsx`는 이번 라운드에 변경하지 않았다(diff 없음).
- **참고(중요)**: 이번에 사용자가 보내준 스크린샷은 실제 기기 캡처가 아니라 내 프리뷰 이미지였던 것으로 확인됐다 — 실제 기기에서 완성된 버튼 왼쪽 캡에 동일한 문제가 없는지는 아직 실물로 재확인하지 못했다. 다음 실기기 빌드에서 최종 확인 필요.
- **롤백 안내**: 이번 라운드는 `src/components/HeroCtaMorph.tsx`에 변경사항이 없으므로 롤백 대상 코드 커밋이 없다(143번 커밋 `72c51fe` 상태 그대로 유지). `QA_LOG.md`만 갱신되었다.

### 145. [하단 탭바] "로또 연구소" 탭 선택 시 아이콘 색상이 거의 안 바뀌던 문제 수정 — grey/color 두 버전의 색상 차이가 실제로 너무 작았음
- **배경**: 사용자가 "하단의 로또 연구소 탭을 눌렀을 때 아이콘 색상이 정확하게 바뀌지 않는다"고 지적했다. 탭바 아이콘은 `app/(tabs)/_layout.tsx`의 `TabIcon` 컴포넌트가 `focused` 여부에 따라 회색 PNG(`grey-lab.png`)와 컬러 PNG(`lab.png`) 두 장을 통째로 바꿔 끼우는 방식이다(래스터 이미지라 동적 tint 불가 — 93번 원복 때 정한 구조, 로직 자체는 정상 동작).
- **원인**: 로직이 아니라 **에셋 자체의 색상 차이가 육안으로 거의 구분 안 될 만큼 작았다**. 두 PNG의 알파>200 영역 평균 RGB와 채도를 직접 계산해보니: `grey-lab.png`는 순수 무채색(115.7, 115.7, 115.7, 채도 0)인데, "선택" 버전인 `lab.png`는 (104.4, 117.1, 137.5)로 채도가 겨우 0.245에 불과했다 — 코드 주석에는 "선택 시 다크 네이비(brand.dark 계열)를 원한다"고 돼 있었지만 실제로는 grey 버전과 밝기·채도가 비슷한 옅은 청회색이었다. 비교를 위해 다른 3개 탭의 컬러 버전도 같은 방식으로 측정했더니 `generate.png` 채도 0.589, `tickets.png` 채도 0.622로 lab만 확연히 낮았다 — 즉 "번호 만들기"/"내 번호" 탭은 선택 시 색이 뚜렷하게 바뀌는데 "로또 연구소"만 유독 변화가 잘 안 보였던 것이다.
- **작업**: `lab.png`(선택 상태 컬러 버전)를 HSV 공간에서 재작업했다 — 원본의 V(명도) 채널은 그대로 유지해 기존 막대그래프 아이콘의 입체 음영(막대마다 밝은 면/어두운 면)은 그대로 보존하면서, H(색상)와 S(채도)만 `theme/colors.ts`의 `brand.dark`(`#0F172A`, HSV로 환산 시 색상 222.2°/채도 0.643)에 맞춰 전체적으로 재지정했다. 그 결과 평균 RGB가 (46.0, 68.5, 121.6), 채도 0.622로 바뀌어 다른 탭들(0.589~0.622)과 비슷한 수준의 뚜렷함을 갖게 됐다. `grey-lab.png`는 그대로 뒀다(원래도 문제없는 무채색).
- **검증**: (1) 변경 전/후 평균 RGB·채도를 수치로 비교해 grey 버전 및 다른 3개 탭 컬러 버전과 나란히 확인. (2) 실제 탭바 배경색(라이트 `#FFFFFF`/다크 `#161F32`, `theme/colors.ts` 기준) 위에 합성한 미리보기로 육안 대비 확인 — 라이트/다크 모드 모두에서 grey↔navy 전환이 명확하게 보임. (3) 실제 탭바 아이콘 표시 크기(26px)로 축소해서도 색 차이가 또렷하게 구분되는지 확인(200px 원본만 확인하면 축소 시 흐려져 구분이 안 될 수 있어 실제 크기로 별도 확인). (4) 이미지 크기(200×200, RGBA) 포맷은 그대로 유지했고 파일 용량도 2836B→2640B로 오히려 약간 줄었다 — 새 에셋 추가 없이 기존 파일 내용만 교체.
- **롤백 안내**: `assets/tab-icons/lab.png`를 이번 커밋 이전 상태로 되돌리면 원래의 옅은 청회색(채도 0.245) 버전으로 복귀한다.

### 146. [검증 전용] AI 조합 탐색 — 3만 회/10만 회 탐색 속도 저하 여부 점검(A/B 벤치마크) — 저하 없음, 코드 변경 없음
- **요청**: "번호 만들기 > AI 조합 탐색 > 탐색강도 3만 회/10만 회의 경우 속도가 느려지지 않았는지" 확인해달라는 요청.
- **1차 점검 — 최근 변경 이력 확인**: 이 경로의 실제 연산을 담당하는 `src/lib/lottery/{generator,scoring,similarity,pattern,random}.ts` 5개 파일의 최종 수정 시각을 확인한 결과 전부 약 한 달 전 시점에 멈춰 있었다(이번 세션에서 진행한 히어로 CTA 애니메이션·탭바 아이콘 작업은 이 파일들을 전혀 건드리지 않음). 그중 "정확히 3만 회/10만 회에서만" 동작이 달라지는 유일한 최근 변경은 44번 항목의 "끝수 스프레드 최적화"뿐이었다(100만 회·바로생성은 44번에서 명시적으로 제외됨) — 사용자가 질문한 범위와 정확히 일치해 가장 유력한 조사 대상으로 판단.
- **2차 점검 — 추측 대신 A/B 벤치마크로 실측**: `generator.ts`/`scoring.ts` 등을 그대로 Node(tsx)로 뽑아 두 벌 준비 — (A) 현재 코드 그대로, (B) `scoring.ts`의 `isLastDigitSpreadOptimizationActive()`가 항상 `false`를 반환하도록 임시 패치해 44번 이전 상태를 재현. 동일 환경(같은 머신, 같은 실행 세션)·동일 입력(저장번호 20개, 인기번호 회피 on, `avoidMySavedNumbers` on 등 `ai-search.tsx`의 기본값과 동일 조건)으로 3만/10만 회 각각 7회씩 실행해 중앙값 비교.
  - 3만 회: (B)없음 449.2ms / (A)있음 412.0ms
  - 10만 회: (B)없음 1391.0ms / (A)있음 1413.5ms
  - 두 경우 모두 차이가 1~2% 수준으로 반복 실행 간 자연 변동(노이즈) 범위 안이었다 — "끝수 스프레드 최적화"는 번호 6개의 끝수만 세는 O(6) 연산이라 예상대로 측정 가능한 성능 영향이 없음을 확인.
- **결론 — 코드 변경 없음**: 3만 회/10만 회 탐색 속도는 저하되지 않았다. 이 경로에 최근 변경된 코드가 없고, 유일하게 의심되던 44번 변경도 A/B 벤치마크로 영향 없음을 직접 확인했으므로 이번 요청은 점검만으로 종결.
- **참고(한계)**: 이번 실측은 데스크톱 Node(tsx) 기준이며, 42번 항목의 벤치마크 방법론과 동일한 접근을 재사용했다. 실기기(Hermes) 절대 체감 속도를 그대로 대변하진 않지만, "느려졌는지 여부"를 가리는 상대 비교(A vs B, 동일 환경) 목적에는 이 방법으로 충분하다고 판단했다.
- **롤백 안내**: 이번 라운드는 코드 변경이 없으므로 롤백 대상이 없다.

### 147. QR 당첨 확인 — 카메라 권한 거부 시 "설정으로 바로 이동" 버튼 추가
- **요청**: 카메라 권한이 거부된 상태(다시 묻지 않기 등으로 OS가 더 이상 권한 팝업을 띄워주지 않는 상태)에서 "QR 당첨 확인" 화면에 진입하면, "설정 앱에서 카메라 권한을 직접 허용해주셔야 해요. (설정 > 앱 > 금손로또 > 권한)"라는 안내 텍스트만 보여주고 있었다. 주요 앱들처럼 그 설정 화면으로 바로 연결해주는 버튼이 있어야 한다는 요청 — 사용자가 직접 설정 앱을 열고 경로를 찾아가야 하는 지금 방식은 텍스트 안내만으로는 실제 실행까지 이어지기 번거롭다는 지적.
- **원인**: `app/generate/qr-check.tsx`의 권한 거부 화면에서 `permission.canAskAgain`이 `false`(OS가 더 이상 네이티브 권한 팝업 자체를 안 띄워주는 상태 — 한 번 거부했거나 "다시 묻지 않음"을 선택한 경우)일 때는 `requestPermission()`을 다시 호출해도 아무 효과가 없어서, 이 경로에서만 버튼 대신 안내 텍스트로 대체돼 있었다. `canAskAgain`이 `true`인 최초 상태에는 이미 "카메라 권한 허용" 버튼이 있어 원인이 없었다.
- **수정**: `canAskAgain === false` 분기에 `Linking.openSettings()`(React Native 코어 API — 파일에 이미 `Linking`이 import돼 있었음, iOS/Android 모두 해당 앱의 설정 화면을 바로 연다)를 호출하는 "설정에서 권한 허용하기" 버튼을 추가했다. 기존 안내 텍스트("설정 > 앱 > 금손로또 > 권한")는 설정 화면 도착 후 "권한" 메뉴를 찾는 데 여전히 도움이 되므로 버튼과 함께 남겨뒀다. `openSettings()`가 실패하는 극히 드문 경우(일부 구형 기기 등)를 대비해 실패 시 Alert로 폴백 안내를 띄우도록 했다.
- **점검 범위 확인**: 앱 전체에서 `canAskAgain`/"설정 앱에서"/"권한을 직접" 패턴을 grep으로 검색해, 카메라 권한을 요청하는 화면이 이 QR 당첨 확인 화면 한 곳뿐임을 확인했다 — 다른 화면에 동일 패턴이 남아있지 않음.
- **검증**: 수정한 파일의 중괄호/소괄호 균형 확인(균형 정상). `Linking`은 새 import 없이 기존 import를 그대로 재사용했다(새 의존성 추가 없음). `Linking.openSettings()`는 React Native 코어 API라 별도 네이티브 모듈 설치가 필요 없다. 실기기에서 카메라 권한을 거부(다시 묻지 않음 포함)한 상태로 QR 당첨 확인 화면에 진입해 버튼이 실제로 앱 설정 화면으로 이동시키는지 확인 필요.
- **롤백 안내**: `app/generate/qr-check.tsx`를 이번 커밋 이전 상태로 되돌리면 텍스트 안내만 있던 이전 버전으로 복귀한다.

### 148. [긴급/치명적] EAS 프리뷰 빌드 설치 직후 앱이 즉시 종료(크래시)되던 문제 수정 — 히어로 CTA 버튼의 `Pressable`에 애니메이션 값을 직접 넣은 것이 원인
- **배경**: 147번까지 반영한 상태로 `eas build --platform android --profile preview`로 빌드해 실기기에 설치한 뒤 앱 아이콘을 누르니 열리자마자 바로 종료됐다. 이전 버전 앱을 완전히 삭제하고 재설치해도 동일하게 재현돼 서명/잔여 설치 문제가 아님을 먼저 배제했다.
- **진단 과정**: 스탠드얼론 APK는 크래시 리포팅 SDK가 없어(Sentry 등 미설치) 원인 로그를 얻을 수 없었다. (1) 최근 수정한 3개 파일(`HeroCtaMorph.tsx`, `qr-check.tsx`, `app/(tabs)/_layout.tsx`) 전부 `ts.transpileModule`로 문법 오류 여부 확인 — 전부 정상. (2) 145번에서 교체한 `lab.png`를 PIL `.verify()`+청크 파싱으로 에셋 손상 여부 확인 — 정상. (3) `app.json`/`package.json` 재검토 — 플러그인 설정에 이상 없음. 이 세 가지로 원인이 좁혀지지 않아, 더 상세한 오류를 보기 위해 로컬 PC에서 `npx expo start` + Expo Go로 같은 코드를 실행해보기로 했다. 이 과정에서 이전에 중단된 `npm install`이 남긴 잠긴 임시 파일(`node_modules\.bin\.nanoid-...`) 때문에 `EACCES` 오류가 나서, `node_modules`를 삭제(`.gitignore`에 등록돼 git 추적 대상이 아니므로 삭제해도 안전함을 먼저 확인)하고 재설치해 Expo Go 구동에 성공했다. Expo Go는 실제 크래시를 네이티브 레벨이 아니라 JS 레벨의 붉은 오류 화면과 전체 스택 트레이스로 보여줬고, 여기서 정확한 원인을 특정할 수 있었다: `Invariant Violation: Transform with key of "scale" must be a number: {"scale":1}`, 스택 트레이스가 `HeroCtaMorph.tsx` → `HomeScreen` → `TabsLayout`을 정확히 가리켰다.
- **원인**: `HeroCtaMorph.tsx`의 히어로 CTA 히트 영역이 `react-native`의 일반 `Pressable`(애니메이션 인식 컴포넌트가 아님)을 그대로 사용하면서 `style={[styles.hitArea, { transform: [{ scale: pressScale }] }]}`처럼 `pressScale`(순수 `Animated.Value` 객체, 숫자가 아님)을 transform에 직접 넣고 있었다. 일반 `Pressable`/`View`는 `Animated.View`/`Animated.Text`와 달리 style prop을 Animated의 네이티브 바인딩 경로로 연결해주지 않기 때문에, RN이 렌더링 시 transform 값이 실제 숫자로 해석되길 기대하는 `processTransform.js`의 `_validateTransform` 단계에서 `Animated.Value` 객체를 그대로 만나 이 Invariant Violation을 던지며 앱 전체가 죽는다. Expo Go(SDK 54)는 `app.json`의 `"newArchEnabled": false` 설정과 무관하게 New Architecture(Fabric)를 강제로 켜기 때문에 이 오류가 JS 레벨 red screen + 전체 스택 트레이스로 뚜렷하게 드러났고, 반면 `newArchEnabled: false`를 그대로 따르는 실제 스탠드얼론 APK(Old Architecture/Paper)는 같은 근본 원인으로 네이티브 레벨에서 조용히(로그 없이) 죽었을 가능성이 매우 높다 — 두 환경 모두 동일한 오용이 원인.
- **수정**: `const AnimatedPressable = Animated.createAnimatedComponent(Pressable);`를 정의해 `Pressable`을 Animated 인식 컴포넌트로 감싼 뒤, 히어로 CTA 히트 영역의 여닫는 태그를 `<Pressable ...>`/`</Pressable>`에서 `<AnimatedPressable ...>`/`</AnimatedPressable>`로 교체했다(내부 props인 `style`/`onPress`/`onPressIn`/`onPressOut`/`accessibilityRole` 등은 그대로 유지). 이렇게 하면 `pressScale`(`Animated.Value`)이 다른 `Animated.View`/`Animated.Text`와 동일한 방식으로 네이티브 Animated 바인딩을 통해 정상적으로 처리된다.
- **점검 범위 확인 — 같은 유형의 버그가 다른 곳에도 있는지 전체 재조사**: 프로젝트 전체 `src/`, `app/` 디렉토리를 다시 나열해 아직 로컬에 없던 나머지 `.tsx` 파일(컴포넌트 19개, 화면 15개)까지 전부 내려받아 (1) `transform: [{ ... }]`에 변수를 넣는 모든 위치를 grep으로 검색해 전부 `Animated.View`/`Animated.Text`/`AnimatedPressable`(이번에 고친 것 포함) 위에서만 쓰이고 있음을 확인, (2) `Animated.Value`/`Animated.ValueXY`를 만드는 모든 위치(`exclusion.tsx`의 토스트, `DeepPatternLoadingBoard.tsx`의 SVG 진행률, `SettingsSheet.tsx`의 바텀시트 슬라이드, `Skeleton.tsx`의 스켈레톤 깜빡임, `Dice45.tsx`의 회전/스케일/글로우, `LottoBallLoader.tsx`의 그림자, `SumTrendChart.tsx`의 스크롤 인디케이터)를 각각 추적해 전부 `Animated.View`/`Animated.Text`나 `Animated.createAnimatedComponent`로 감싼 SVG 컴포넌트(`AnimatedPath`/`AnimatedCircle`) 위에서만 쓰이고 있음을 확인했다. 즉 이번에 고친 `HeroCtaMorph.tsx`의 히트 영역이 프로젝트 전체에서 유일한 위반 사례였다.
- **검증**: 수정한 `HeroCtaMorph.tsx`의 중괄호 133/133, 소괄호 258/258 균형 확인. `AnimatedPressable`이 정확히 3곳(정의 1 + 여는 태그 1 + 닫는 태그 1)에서만 쓰이고 일반 `<Pressable`/`</Pressable>`은 이 파일에 더 이상 남아있지 않음을 grep으로 확인. `ts.transpileModule` 문법 검사 재통과. 사용자가 이미 로컬에서 `npx expo start`를 띄워둔 상태이므로, 이번 수정을 반영한 뒤 Metro 핫리로드로 Expo Go에서 즉시 재현·확인 가능 — 스탠드얼론 APK 재빌드 없이 가장 빠르게 크래시 해소를 확인할 수 있는 경로.
- **참고(별개 사항, 이번에 수정하지 않음)**: `npx expo start` 실행 시 "New Architecture가 Expo Go에서는 항상 켜져 있는데 `app.json`에는 `newArchEnabled: false`로 명시돼 있다"는 경고가 함께 출력됐다. 이는 이번 크래시의 원인이 아니라(원인은 위의 `Pressable`+`Animated.Value` 오용) 별개의 기존 설정 이슈이며, 이번 라운드에서는 변경하지 않았다 — 필요 시 별도로 논의 필요.
- **롤백 안내**: `src/components/HeroCtaMorph.tsx`를 이번 커밋 이전 상태로 되돌리면 일반 `Pressable`을 그대로 쓰던(크래시가 재현되는) 버전으로 복귀한다.

### 149. [히어로 CTA 조사] "구체→CTA 버튼 모프가 안 보인다" 신고 — 실제로는 정상 재생 중이었고(1.2~1.5초라 놓치기 쉬움), 대신 조사 중 별도의 눌림 애니메이션 버그(네이티브/JS 드라이버 혼용)를 발견해 수정
- **배경**: 148번(크래시 수정) 반영 후 사용자가 EAS 프리뷰 APK를 실기기에 설치해 테스트한 결과, 앱은 정상적으로 열리지만 "구체→CTA 버튼으로 형태가 바뀌는 모프가 전혀 안 보이고, 처음부터 완성된 버튼 상태로 보인다. 누를 때 느낌만 있다"고 보고했다. 이 모프는 사용자가 "유저에게 충분히 임팩트 있는 기능"이라고 강조한, 135~147번에 걸쳐 여러 차례 다듬어온 핵심 연출이라 원인 파악이 중요했다.
- **1차 가설 — Reduce Motion(기각)**: 코드상 `AccessibilityInfo.isReduceMotionEnabled()`가 true면 애니메이션 없이 즉시 완성 상태로 건너뛰도록 돼 있어, 접근성 "애니메이션 제거"나 개발자 옵션의 애니메이션 배율(창/전환/Animator 길이 배율)이 0으로 설정된 게 원인일 가능성을 먼저 짚었다. 사용자가 실기기에서 두 설정을 전부 껐음에도 동일하게 재현돼 이 가설은 기각했다.
- **2차 조사 — 실기기 로그로 직접 확인**: 추측 대신 실제로 어느 분기에서 멈추는지 확인하기 위해 `playNow()`/`useFocusEffect`/`handleTrackLayout`에 임시 `console.log`를 추가했다. Expo Go(SDK 54 버전 재설치 필요 — Play 스토어의 Expo Go가 그새 SDK 57로 자동 업데이트돼 있어 호환 안내를 따라 SDK 54용을 별도 설치)로 1차 확인한 결과 애니메이션 시퀀스가 "시작"부터 "완료"까지 정상적으로 로그가 찍혔다. 다만 Expo Go는 New Architecture(Fabric)를 강제로 쓰기 때문에, 실제 프리뷰 빌드(`app.json`의 `newArchEnabled:false`를 따라 Old Architecture로 빌드됨)와 아키텍처가 달라 완전히 같은 조건이라 보기 어려웠다. 그래서 `eas.json`에 이미 있던 "development" 프로필(`expo-dev-client` 설치 후 `eas build --profile development`)로 Old Architecture를 유지하면서도 Metro에 연결해 로그를 볼 수 있는 개발 빌드를 새로 만들어 재확인했다 — 결과는 동일하게 `useFocusEffect` → 레이아웃 대기/확보 → `playNow()` → `isReduceMotionEnabled()=false` → "애니메이션 시퀀스 시작" → "완료"까지 매번 정확한 순서로 실행됐다.
- **결론 1 — 모프 로직 자체는 처음부터 정상**: 실제 프리뷰 빌드와 동일한 아키텍처(Old Architecture)에서도 로직이 완전히 정상 작동함을 로그로 확인했다. 반짝임(220ms)+모프(spring, 약 1초)+완성 펄스(약 0.4초)를 합친 전체 재생 시간이 1.2~1.5초로 짧아서, 앱 아이콘을 누르고 화면에 시선이 닿기 전에 이미 끝나버려 놓쳤을 가능성이 가장 높다고 판단했고, 같은 개발 빌드로 앱을 완전히 새로 켜서 화면에 집중해서 지켜본 결과 사용자가 직접 재생되는 것을 확인했다("쫌 빠르더라. 확인됐어!").
- **결론 2 — 조사 중 발견한 별도의 실제 버그**: 로그를 받는 과정에서 버튼을 누를 때(`onPressIn`) `ERROR Style property 'width' is not supported by native animated module`이 반복적으로 찍히는 것을 확인했다. 원인은 눌림 피드백용 `pressScale`(6~7차에서 추가)이 `useNativeDriver: true`로 재생되는데, 이 값에서 파생된 `pressDim`이 글로우 링/그림자 레이어의 `opacity`에도 쓰이고 있고, 그 레이어들의 같은 스타일 객체 안에는 `width`(`progress` 기반, `useNativeDriver` 미지원)도 함께 있다는 것이었다 — `pressScale`을 네이티브로 승격시키려는 순간 RN이 같은 스타일 객체의 `width`까지 함께 네이티브로 옮기려다 이 오류를 던진다. 이 검증은 `__DEV__` 전용이라 실제 릴리즈 빌드에서는 경고 없이 조용히 실패했을 가능성이 있어(버튼을 처음 누르는 순간부터 애니메이션 그래프가 깨질 위험) 잠재적으로 심각한 버그였다.
- **수정**: `pressScale`의 두 `Animated.timing`(누를 때/뗄 때) 호출을 `useNativeDriver: true`에서 `false`로 변경했다 — 이 파일의 다른 모든 애니메이션 값(`progress`/`shine`/`settle`)이 이미 전부 `useNativeDriver: false`로 통일돼 있던 것과 같은 방식으로 맞춰, 네이티브/JS 드라이버가 한 스타일 그래프 안에서 섞이는 근본 원인을 제거했다.
- **검증**: 조사에 쓰인 임시 `console.log`(11곳)는 전부 제거하고 원래 코드 스타일로 정리했다(중괄호 133/133, 소괄호 281/281 균형 확인, `ts.transpileModule` 문법 검사 통과, 코드 내 남은 `console.log`는 이 조사 내역을 설명하는 주석 텍스트 2곳뿐이고 실제 코드에는 없음을 grep으로 확인). 수정한 `pressScale` 변경 사항은 위 개발 빌드(Old Architecture)에서 버튼을 눌러 재확인이 필요하다 — 아직 이 수정 반영 후의 눌림 동작 자체는 실기기 재확인 전이다.
- **참고**: 이 조사 과정에서 Play 스토어 Expo Go가 SDK 57로 자동 업데이트되며 SDK 54 프로젝트와 호환이 깨지는 것을 확인했다 — 프로젝트 코드와 무관한 Expo Go 자체의 특성(항상 최신 SDK 하나만 지원)이며, `expo.dev/go?sdkVersion=54`에서 SDK 54용을 재설치하면 된다(다만 Play 스토어 버전과 서명이 달라 기존 Expo Go를 먼저 삭제해야 충돌 없이 설치됨).
- **롤백 안내**: `src/components/HeroCtaMorph.tsx`를 이번 커밋 이전 상태로 되돌리면 `pressScale`이 `useNativeDriver: true`였던(눌렀을 때 잠재적으로 네이티브 검증 오류가 날 수 있는) 버전으로 복귀한다.

### 150. [번호 만들기 > AI 조합 탐색] 생성 결과 화면 — 저장 전인데 "최종 저장 조합"이라고 표시되던 라벨을 "생성된 조합"으로 변경
- **요청**: 사용자가 생성 결과 화면 스크린샷에 표시해 지적했다 — "최종 저장 조합" 항목이 실제로 "번호 저장" 버튼을 누르기도 전에, 결과가 생성되자마자 바로 표시되고 있어서 마치 이미 저장된 것처럼 오해할 수 있다는 지적. 함께, EV 최적화/휠링/스프레드 같은 용어가 일반 사용자에게는 어려울 수 있어 도움말을 제공하면 좋겠다는 의견도 받았으나, 이번 라운드는 "우선 1번만 변경 진행"하라는 명시적 지시에 따라 라벨 변경만 처리했다.
- **원인**: `src/components/ProbabilityCard.tsx`(생성 결과 화면 `app/generate/result.tsx`에서만 쓰이는 공유 컴포넌트)의 마지막 행이 `label="최종 저장 조합"`으로 하드코딩돼 있었다. 이 카드는 `result.tsx`에서 결과가 생성되는 즉시(`<ProbabilityCard probability={lastResult.probability} .../>`) 렌더링되고, "번호 저장" 버튼(`handleSave(game, "SAVED")`)은 그 아래 별도 푸터 버튼으로 있어 사용자가 직접 눌러야만 실제 저장이 일어난다 — 즉 이 시점에는 아직 아무것도 저장되지 않은 "생성된 후보"일 뿐인데 라벨만 "최종 저장"이라고 표현하고 있었다.
- **수정**: 해당 `Row`의 `label`을 `"최종 저장 조합"` → `"생성된 조합"`으로 변경했다. 값 자체(`probability.uniqueGameCount`게임)는 그대로 유지 — 실제로 이번에 생성된 고유 조합 개수를 보여주는 값이라 라벨만 바로잡으면 되는 문제였다.
- **점검 범위 확인**: `ProbabilityCard`가 프로젝트 전체에서 `app/generate/result.tsx` 한 곳에서만 쓰이고 있음을 grep으로 확인했다 — 다른 화면에 영향 없이 이 한 곳만 정확히 고쳐진다.
- **검증**: `ts.transpileModule` 문법 검사 통과. 변경 범위가 문자열 라벨 하나뿐이라 레이아웃/로직에는 영향이 없다.
- **보류(다음 라운드 예정, 이번엔 변경 안 함)**: "EV 최적화", "휠링 방식 분산", "끝수 스프레드 최적화" 같은 칩 라벨이 일반 사용자에게 생소할 수 있다는 피드백 — 사용자가 "우선은 기록만 해놓고 나중에 수정하자"고 명시했으므로, 이번 라운드에서는 코드를 건드리지 않고 여기 기록만 해둔다. 다음에 진행할 때는 각 칩에 짧은 도움말(툴팁/바텀시트 등)을 붙이는 방향이 될 것으로 보인다.
- **롤백 안내**: `src/components/ProbabilityCard.tsx`를 이번 커밋 이전 상태로 되돌리면 "최종 저장 조합" 라벨로 복귀한다.

### 151. QR 당첨 확인 — "권한이 켜지는 화면으로 바로 가게 해달라"는 요청 조사 결과(기술적 한계 확인) 및 설정 화면 도착 후 남은 단계를 장년층도 헷갈리지 않게 안내하는 단계별 박스로 개선
- **요청**: 사용자가 "홈탭 QR당첨확인에서 권한을 바로 가게 해줘야하는데 바로 가는 위치를 딱 조정만 하면 되게끔 유저 편의성을 극대화해야할 거 같아. 장년층도 사용하는 앱이므로 이런 편의성이 중요해"라고 요청했다. 147번에서 추가한 "설정에서 권한 허용하기" 버튼이 실제로는 "앱 정보" 화면(스크린샷 1)에서 멈추고, 카메라 권한이 바로 켜지는 "앱 권한" 화면(스크린샷 2)까지는 한 번 더 탭해야 한다는 지적.
- **조사 — "앱 권한" 화면이나 개별 권한(카메라) 토글 화면으로 직접 이동하는 공식 방법이 있는지 확인**: `Linking.openSettings()`는 Android의 `ACTION_APPLICATION_DETAILS_SETTINGS`(공식 문서화된 유일한 안정적 액션)를 여는데, 이게 정확히 스크린샷 1("애플리케이션 정보")이다. 더 깊은 화면(앱 권한 목록, 또는 카메라 권한 하나만)으로 바로 이동하는 공식 Intent가 있는지 웹 검색으로 확인한 결과: (1) 이 분야에서 가장 널리 쓰이는 오픈소스 라이브러리 `react-native-permissions`(zoontek/react-native-permissions, GitHub)의 `openSettings()`조차 `application`/`alarms`/`notifications`/`fullscreen` 카테고리만 지원하고, 개별 런타임 권한(카메라·마이크 등) 화면은 지원하지 않는다고 문서에 명시돼 있다 — 지원하지 않는 카테고리를 요청하면 `application`으로 자동 폴백한다. (2) 다른 기술 자료(code.luasoftware.com)도 "앱 권한 화면을 직접 열 수는 없고, `ACTION_APPLICATION_DETAILS_SETTINGS`로 앱 설정 화면을 연 뒤 그 안에서 권한 메뉴로 접근해야 한다"고 명시한다. 비공식/미문서화된 Intent(예: `ACTION_MANAGE_APP_PERMISSION`)를 억지로 시도하는 방법이 있긴 하지만, 공식 API가 아니라 기기 제조사(삼성 One UI 등)·Android 버전마다 조용히 실패할 수 있어 — 잘 되면 편리하지만 안 될 때는 아무 반응이 없어 오히려 장년층 사용자에게 더 큰 혼란을 줄 수 있다고 판단해 적용하지 않았다.
- **결론 — 기술적으로 더 깊이 들어가는 건 불가능, 대신 남은 단계를 명확하게**: "앱 정보" 화면 도착까지는 이미 원탭(147번)으로 최적화돼 있고, 그 이후 "권한 → 카메라 켜기" 두 번의 탭은 서드파티 앱이 건너뛸 수 있는 공식적인 방법이 없다. 그래서 이번에는 이 두 번의 탭을 사용자가 절대 헷갈리지 않도록 안내를 크게 개선하는 방향으로 진행했다.
- **수정**: 기존에는 작은 글자(13px) 한 줄로 "설정 > 앱 > 금손로또 > 권한 경로에서 카메라를 켜주세요"라고만 안내했는데, 이를 번호가 매겨진 단계별 안내 박스로 교체했다 — "1. 권한 누르기", "2. 카메라 눌러서 켜기" 두 단계를 각각 원형 숫자 배지 + 16px 본문 텍스트로 표시하고, 실제로 눌러야 할 단어("권한", "카메라")는 브랜드 색상 + 굵게로 강조해 화면을 훑기만 해도 다음에 뭘 눌러야 할지 바로 파악되도록 했다(`app/generate/qr-check.tsx`).
- **검증**: `ts.transpileModule` 문법 검사 통과, 중괄호 153/153·소괄호 97/97 균형 확인. 기존 버튼 로직(`Linking.openSettings()` + 실패 시 Alert 폴백)은 그대로 유지했고, 새 단계별 박스는 `canAskAgain === false`(설정 화면으로 유도해야 하는 경우)에만 표시되므로 기존 최초 권한 요청 흐름(`canAskAgain === true`)에는 영향이 없다. 실기기에서 실제로 "설정에서 권한 허용하기" 버튼을 눌러 단계별 박스가 정상적으로 보이는지, 안내한 순서대로 눌렀을 때 카메라 권한이 켜지는지 확인 필요.
- **참고**: 완전한 "원탭으로 카메라 권한 토글까지" 요청은 Android/iOS 플랫폼 자체의 보안상 제약으로 서드파티 앱에서는 구현 불가능함을 확인했다 — 이는 이 앱만의 한계가 아니라 업계 전반의 표준 라이브러리에서도 동일하게 확인되는 제약이다.
- **롤백 안내**: `app/generate/qr-check.tsx`를 이번 커밋 이전 상태로 되돌리면 단계별 박스 대신 작은 한 줄 안내 문장이었던 버전으로 복귀한다.

### 152. [하단 탭바] "로또 연구소" 탭 선택 색이 군청색으로 튄다 — 번호 만들기/내 번호와 같은 보라 계열로 재조정
- **요청**: 사용자가 홈 화면 스크린샷에 "로또 연구소" 탭을 표시해 지적했다 — 선택했을 때 아이콘/글자 색이 군청색 계열로 보이는데, 옆의 "번호 만들기"·"내 번호" 탭을 선택했을 때는 보라색 계열이라 통일감이 없다는 지적.
- **원인**: 145번에서 "로또 연구소" 선택 아이콘(`lab.png`)의 색이 grey 버전과 거의 구분이 안 된다는 문제를 고치면서, 당시 코드 주석에 남아있던 "선택 시 다크 네이비(brand.dark)를 원한다"는 이전 결정을 따라 H/S를 `brand.dark`(#0F172A, HSV H=222.2°) 쪽으로 재지정했었다. 실측해보니 이번에 지적받은 `generate.png`(H=260.1°, S=0.581)·`tickets.png`(H=259.7°, S=0.614)는 둘 다 보라 계열인데, `lab.png`만 H=222.2°(남색)로 확연히 달라 세 탭을 나란히 보면 "로또 연구소"만 색조가 다르게 보였다.
- **수정**: 145번과 같은 방식(원본의 V(명도) 채널은 픽셀 단위로 그대로 유지해 막대그래프 아이콘의 입체 음영은 보존, H/S만 교체)으로 `lab.png`를 다시 재작업했다. 이번엔 목표 색상을 `brand.dark`가 아니라 `generate.png`/`tickets.png` 두 아이콘의 실측 평균값(H≈260°, S≈0.6)으로 맞춰, 세 탭의 선택 색이 같은 보라 계열이 되도록 했다. `grey-lab.png`(미선택 상태)는 이번에도 그대로 뒀다.
- **검증**: 변경 전/후 HSV 실측값을 수치로 비교(새 `lab.png` 평균 H=259.8°/S=0.604 — `generate.png`(260.1°/0.581)·`tickets.png`(259.7°/0.614)와 사실상 일치). PIL로 실제 탭바 배경색(라이트 `#FFFFFF`/다크 `#161F32`) 위에 세 아이콘(로또 연구소/번호 만들기/내 번호)을 나란히 합성한 미리보기로 육안 비교한 결과, 라이트/다크 모드 모두에서 세 탭이 같은 보라 계열로 자연스럽게 통일돼 보임을 확인했다. 이미지 포맷(200×200, RGBA)은 그대로 유지, PNG 구조 `verify()` 통과, 파일 용량은 2640B→2403B로 오히려 줄었다(새 에셋 추가 없이 기존 파일 내용만 교체). `app/(tabs)/_layout.tsx`의 설명 주석도 145번 당시의 "다크 네이비를 원함" 기록이 이제 최신 상태가 아니므로, 이번 변경 이력으로 갱신했다.
- **롤백 안내**: `assets/tab-icons/lab.png`를 이번 커밋 이전 상태로 되돌리면 145번의 다크 네이비(군청색) 버전으로 복귀한다.

### 153. [번호 공유] 카카오톡 공유 시 텍스트만 전송됨 — 이미지·제목·설명·"앱에서 확인하기" 버튼이 있는 카드(Feed 템플릿)로 개선
- **요청**: 사용자가 신한은행/신한투자증권·토스 앱의 카카오톡 공유 스크린샷을 첨부하며, 금손로또는 번호를 공유하면 순수 텍스트만 전송되는데 다른 주요 앱들은 이미지·설명과 함께 "앱으로 바로가는" 카드 형태로 공유된다고 지적. "유저 인입에 필수"라며 중요도 높게 요청.
- **조사**: 신한/토스 스크린샷의 카드는 일반적인 OG 태그 링크 미리보기가 아니라 한국 메신저 생태계 특유의 **카카오톡 공유하기(Kakao Link)** 기능일 가능성이 매우 높다고 판단, 두 경로를 조사했다.
  - **경로 A(채택) — 카카오톡 공유하기**: 비공식이지만 활발히 유지보수되는 `@react-native-kakao/share`(`react-native-kakao` 모노레포, v2.4.6) 사용. Expo config plugin 지원, Old Architecture(현재 프로젝트 설정) 지원 확인. Expo Go는 불가하지만 이 프로젝트는 이미 `expo-dev-client`가 세팅돼 있어 새 툴체인 없이 바로 적용 가능.
  - **경로 B(보류) — 범용 OG 태그 링크 미리보기**: 카카오톡 외 채널까지 커버하려면 실제 공개 웹서버(동적 HTML)가 필요한데, 이 프로젝트는 순수 클라이언트 앱이라 그런 백엔드가 전혀 없다(판매점 지도 기능 조사 문서에서도 이미 확인된 사실). 사용자 결정으로 이번에는 보류.
- **카카오 디벨로퍼스 설정** (사용자 본인 계정으로 직접 진행 — 계정 소유자만 가능한 절차):
  - 개인 개발자로 앱 생성(회사명은 사업자등록 없이 자유 텍스트 입력 가능, 카카오 공식 포럼으로 확인). 앱 이름 "금손로또", 앱 ID 1578810.
  - Android 플랫폼 등록: 패키지명 `com.geumsonlotto.app` + 키 해시(`eas credentials`로 확인한 EAS 관리 서명 키스토어의 SHA1 Fingerprint를 Base64로 변환해 등록).
  - iOS는 아직 iOS 개발 자체를 시작하지 않아 이번에는 등록하지 않음(추후 iOS 착수 시 번들 ID `com.geumsonlotto.app`로 등록 예정).
  - 네이티브 앱 키 발급 완료: `457dfeb6e32b6c6f9221a8bbdacfaea1`.
- **브랜딩 이미지 제작**: 카카오톡 Feed 카드용 이미지(800×419 PNG)를 새로 제작 — 앱 아이콘 + "금손로또" + "데이터 분석 전문가가 본인이 쓰려고 만든 앱 / 지금 바로 써보세요" + "로또 번호 생성 · 확률 계산 · 당첨 확인", 배경은 앱 CTA 버튼과 동일한 남색(`brand.dark` #0F172A)→파랑(`brand.primary` #2563EB) 그라디언트로 톤 통일. 사용자와 문구를 함께 다듬었다(최초 "행운의 번호, 지금 확인해보세요" → 사용자가 "전문가가 만든 앱" 서사로 교체 요청 → 다듬는 과정에서 "지금 확인해보세요"가 목적어 없이 붕 뜨는 문제를 지적하고 "지금 바로 써보세요"로 최종 확정, 아래 동적 텍스트와의 문구 중복도 피함). `assets/kakao-share-card.png`로 저장, 기존 당첨번호 데이터(`data/lotto-draws.json`)와 동일한 GitHub 공개 배포 패턴으로 `raw.githubusercontent.com`을 통해 공개 URL 확보·검증 완료: `https://raw.githubusercontent.com/chpark924/ai-lotto-generator/main/assets/kakao-share-card.png` (레포 루트가 `lotto-app` 폴더 자체임을 실측으로 확인 — 처음 시도한 `.../main/lotto-app/assets/...` 경로는 404).
- **구현**:
  - `app.json`: `expo-build-properties` 플러그인에 카카오 Android Maven 저장소(`https://devrepo.kakao.com/nexus/content/groups/public/`) 추가, `@react-native-kakao/core` 플러그인에 네이티브 앱 키 등록(`ios`/`android` 옵션은 빈 객체로 둬 플러그인이 iOS `Info.plist`의 `CFBundleURLTypes`/`LSApplicationQueriesSchemes`를 자동으로 세팅하게 함).
  - `app/_layout.tsx`: 모듈 최상단(RootLayout 마운트 전)에서 `initializeKakaoSDK(네이티브앱키)` 1회 호출 — 기존 `SplashScreen.preventAutoHideAsync()`와 같은 패턴. 초기화 실패해도 앱 자체는 정상 동작하도록 `.catch()`로 조용히 무시.
  - **신규 `src/lib/share/kakaoShare.ts`**: `shareLottoNumbers(title, description, fallbackMessage)` 헬퍼. `@react-native-kakao/share`의 `shareFeedTemplate()`로 이미지·제목·설명·"앱에서 확인하기" 버튼이 있는 Feed 카드를 공유 시도 → `useWebBrowserIfKakaoTalkNotAvailable: true`로 카카오톡 미설치 시 자체적으로 웹 공유창 폴백 → 그마저 실패(reject)하는 예외 상황엔 기존과 동일한 `Share.share({ message: fallbackMessage })`로 한 번 더 폴백. 즉 "공유가 아예 안 되는" 경우가 없도록 3단 폴백 구조.
  - `app/generate/result.tsx`, `app/generate/qr-check.tsx`, `app/(tabs)/tickets.tsx` 3곳의 `handleShare()`를 전부 이 헬퍼 호출로 교체. 각 화면 상황에 맞는 제목/설명을 새로 구성(예: result.tsx는 "내 로또 번호: N·N·N·N·N·N" + 확률 설명, qr-check.tsx는 "제 N회 당첨 결과" + 당첨/낙첨 요약, tickets.tsx는 "내 로또 번호 (상태)" + 번호). `fallbackMessage`는 기존에 쓰던 메시지 문구를 그대로 재사용해 OS 기본 공유 시트 경로의 동작은 바뀌지 않는다.
- **검증(1차, 정적)**: 5개 수정 파일(`kakaoShare.ts`, `app/_layout.tsx`, `result.tsx`, `qr-check.tsx`, `tickets.tsx`) 전부 중괄호/소괄호/대괄호 균형 검사 통과, `Share` import를 제거한 3개 화면에 남은 참조가 없는지 grep으로 재확인 완료. `app.json`은 JSON 파싱 검증 통과. 이번 세션은 `device_bash`(사용자 PC 원격 터미널)가 9월 8일자 윈도우 업데이트 이슈로 막혀 있어, 패키지 설치(`npm install`)는 사용자가 직접 터미널에서 실행함. 이 시점에는 실제 개발 빌드 재빌드·실기기 테스트가 아직 안 된 상태였다 — 아래 "빌드 오류 및 수정", "최종 검증(실기기)" 참고.
- **참고**: Feed 카드의 "앱에서 확인하기" 버튼은 현재 플레이스토어 URL(`https://play.google.com/store/apps/details?id=com.geumsonlotto.app`)을 가리키는데, 아직 정식 출시 전이라 지금 누르면 404가 뜬다. 패키지명 기준으로 미리 정해둔 값이라 정식 출시 즉시 별도 코드 수정 없이 자동으로 유효해진다 — 출시 시점에 카카오 디벨로퍼스 "플랫폼 키" 화면의 Android 스토어 URL도 함께 등록해줄 것(현재는 "없음"으로 비워둔 상태, QA_LOG 및 프로젝트 문서에 기록해둠).

- **빌드 오류 및 수정 (2026-09-20)**: 위 구현을 커밋한 뒤 `eas build --platform android --profile development`로 재빌드하자 `:react-native-kakao_share:compileDebugKotlin FAILED`로 빌드 자체가 실패했다. 로그를 받아 원인을 규명한 결과, `@react-native-kakao/share@2.4.6` 패키지 자체의 버그였다 — React Native 0.81.0에서 `ReactContextBaseJavaModule`이 Java→Kotlin으로 재작성되면서 Kotlin 서브클래스의 `getCurrentActivity()`→`currentActivity` 프로퍼티 축약 문법이 깨지는 업계 전반의 회귀 버그(이 프로젝트나 Old/New Architecture 설정과 무관, 다른 무관한 라이브러리 3곳에서도 동일 이슈 확인됨). 패키지 GitHub 소스(git tag `v2.4.6`)를 직접 가져와 대조 확인했고, 2026-09-20 기준 공식 수정 버전은 없었다(npm/GitHub `main` HEAD 모두 확인). `patch-package`로 `RNCKakaoShareModule.kt` 206번 줄 `val context = currentActivity` → `val context = reactApplicationContext.currentActivity`로 수정하는 패치를 만들어 적용(`patches/@react-native-kakao+share+2.4.6.patch`, `package.json`에 `postinstall: patch-package` 스크립트 추가). 이후 재빌드 성공.
- **최종 검증(실기기, 2026-09-20)**: 빌드된 개발 빌드(APK)를 실기기에 설치해 공유 버튼 테스트 중 2차 문제 발견 — 카카오톡 SDK가 `android keyhash mismatched` 에러를 던지며 OS 기본 공유 시트로 조용히 폴백되고 있었다(진단을 위해 `kakaoShare.ts`/`app/_layout.tsx`에 임시로 `console.error` 로그를 추가해 원인 확인, 확인 후 제거). 원인은 카카오 디벨로퍼스 콘솔 설정 실수였다 — 키 해시 값(`w/VfB5RvTkZGo6ZbG1NZPLFTJjg=`)을 실제 검증에 쓰이는 "키 해시" 필드가 아니라, 단순 라벨용인 "키 이름" 필드에 잘못 입력해뒀던 것(두 필드가 UI상 이름이 비슷해 헷갈리기 쉬움). 사용자가 정확한 "Android 키 해시" 입력창에 값을 넣고 "+"로 추가 후 저장하자 정상화됨. 추가로 테스트 기기의 카카오톡 앱 자체가 로그인이 안 돼 있어 공유 시 카카오톡 로그인 화면이 뜬 적이 있었는데, 로그인 후에는 정상 — 이건 앱 코드와 무관한 테스트 기기 상태였다. 최종적으로 카카오톡 채팅방에 이미지·제목·설명·"앱에서 확인하기" 버튼이 있는 Feed 카드가 정상적으로 전송되는 것을 사용자가 직접 확인함. **번호 생성 결과 화면 기준으로 확인 완료** — QR 당첨 확인/내 번호(tickets) 화면도 동일한 `shareLottoNumbers` 헬퍼를 공유하므로 정상 동작이 기대되나, 개별 확인은 하지 않았다(문제 발견 시 재문의하기로 함).
- **롤백 안내**: `app/_layout.tsx`의 `initializeKakaoSDK(...)` 호출 블록 제거, `app.json`의 `@react-native-kakao/core` 플러그인 항목 제거, 3개 화면의 `handleShare()`를 `Share.share({ message: ... })` 직접 호출로 되돌리면(각 파일의 `fallbackMessage` 문자열이 원래 메시지와 동일하므로 그대로 재사용 가능) 이번 커밋 이전 상태로 복귀한다. `src/lib/share/kakaoShare.ts` 파일 자체는 삭제해도 무방.
