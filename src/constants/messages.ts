...
/**
 * 로또 연구소 "이번 회차 번호 이후 통계" 카드 전용 디스클레이머.
 * SUM_TREND_NOTICE와 같은 원칙(§23) — 서술적 통계일 뿐 예측이 아니라는 점, 그리고 회차
 * 독립성 때문에 다음 회차 확률에 영향을 주지 않는다는 점을 함께 명시한다.
 */
export const TRANSITION_FREQUENCY_NOTICE =
  "이 통계는 과거에 특정 번호가 나온 다음 회차에 어떤 번호가 몇 번 나왔는지 집계한 서술적 통계일 뿐입니다. 로또 추첨은 매회 독립적으로 이뤄지므로, 특정 번호가 나왔다고 해서 다음 회차에 특정 번호가 나올 확률이 더 높아지지 않습니다. 표본이 작을수록(트리거 번호별 표본 수를 함께 표시) 이 순위는 우연에 의한 차이일 가능성이 큽니다.";
