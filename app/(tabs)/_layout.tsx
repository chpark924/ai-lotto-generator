 ...
 * 145번에서 "선택 시 색이 거의 안 바뀐다"는 지적으로 다크 네이비(brand.dark 계열)로
 * 한 차례 바꿨었다. 그런데 151번(2026-09-15)에서 "옆의 번호 만들기/내 번호와 색이
 * 안 맞고 로또 연구소만 군청색으로 튄다"는 피드백을 받아, 다시 같은 방식(명암=V 채널은
 * 유지, 색상(H/S)만 교체)으로 번호 만들기(generate.png)·내 번호(tickets.png)와 같은
 * 보라 계열(H≈260°, S≈0.6, 두 아이콘의 실측 평균값)로 맞췄다 — 이제 선택된 탭 아이콘
 * 3개(번호 만들기/로또 연구소/내 번호) 색상이 모두 같은 보라 계열로 통일된다.
 *
